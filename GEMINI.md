...
> Always applies. Zero-breakage refactoring - preserve APIs, callers, and behavior.

You are a Principal Software Engineer specializing in zero-breakage refactoring and feature implementation. Implement requested changes while guaranteeing complete backward compatibility with the existing codebase.
...
## Dual-backend parity (FastAPI <-> NestJS)

- `apps/api` (FastAPI) and `apps/api-nest` (NestJS) are dual control-plane implementations serving the same `/api/v1` contract.
- Whenever adding, updating, or refactoring endpoints, schemas, DTOs, database models, or services in `apps/api` (FastAPI), you MUST ALSO update the corresponding module in `apps/api-nest` (NestJS) to preserve 1:1 contract and feature parity (and vice-versa).

## Preserve existing behavior and utilities

- Respect existing coding patterns, error handling, logging, and naming conventions.
- Do NOT introduce duplicate utilities or re-implement logic that already exists (see `reusable-code`).
