...
def write_local_runtime_iac(
    ...
    *,
    ...
    """Write Terraform/OpenTofu, Pulumi, Ansible, or Launchpad script stubs for local non-K8s runtimes."""
    ...
    if engine in {IaCEngine.LAUNCHPAD, IaCEngine.LAUNCH_SCRIPT}:
        return _write_local_launch_script(workspace_dir, name=name, runtime_mode=runtime_mode)
...
def _write_local_launch_script(
    workspace_dir: Path,
    *,
    name: str,
    runtime_mode: WorkspaceRuntimeMode,
    ...
    down_cmd = (
        "docker compose down"
        if runtime_mode == WorkspaceRuntimeMode.DOCKER_COMPOSE
        else "echo 'Stop local process or container'"
    )

    infra = workspace_dir / "infra"
    infra.mkdir(parents=True, exist_ok=True)
    script_path = infra / "launchProvision.sh"
    content = (
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
        f"# Generated by Launchpad (LaunchProvision) for local runtime ({label})\n"
        "# Usage: bash infra/launchProvision.sh [up|down|outputs|configure]\n"
        'ACTION="${1:-up}"\n'
        f'NAME="{name}"\n'
        'echo "==> LaunchProvision $ACTION ($NAME)"\n\n'
        'if [ "$ACTION" = "configure" ]; then\n'
        "  echo '==> Local runtime ready - no cloud configuration needed'\n"
        "  exit 0\n"
        "fi\n\n"
        'if [ "$ACTION" = "outputs" ]; then\n'
        f'  echo environment_name="$NAME" runtime_mode="{label}" public_ip=127.0.0.1 preview_url=http://127.0.0.1:8080\n'
        "  exit 0\n"
        "fi\n\n"
        'if [ "$ACTION" = "down" ]; then\n'
        f'  echo "==> Local runtime down ($NAME)"\n'
        f"  {down_cmd}\n"
        "  exit 0\n"
        "fi\n\n"
        f'echo "==> Local runtime ready ({howto})"\n'
    )
    ...
    try:
        ...
        pass
    return ["infra/launchProvision.sh"]
...
def _write_local_terraform(
    ...
    *,
    ...
    _write(
        ...
        'output "public_ip" {\n'
        '  value       = "127.0.0.1"\n'
        '  description = "Local runtime public IP"\n'
        "}\n\n"
        'output "preview_url" {\n'
        '  value       = "http://127.0.0.1:8080"\n'
        '  description = "Local runtime preview URL"\n'
        "}\n",
    )
    _write(
        TF_ROOT / ".gitignore",
