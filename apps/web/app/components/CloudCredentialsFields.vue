...
const props = withDefaults(
  defineProps<{
    /** Parent-owned object; fields are edited in place (no v-model / defineModel). */
    credentials: CloudCredentialsForm
    provider: 'gcp' | 'aws' | 'azure' | 'cloudflare'
    /** When true, show a one-off override of Settings keys (collapsed by default). */
...
const keysOpen = ref(!props.overrideMode)
...
const saEmbeddedProjectId = computed(() =>
  projectIdFromSaJson(props.credentials?.gcp_sa_key_json || ''),
)
...
watch(
  () => props.credentials?.gcp_sa_key_json,
  (raw) => {
    if (!props.credentials) return
    const project = projectIdFromSaJson(raw || '')
    if (project && !(props.credentials.gcp_project_id || '').trim()) {
      props.credentials.gcp_project_id = project
    }
  },
)
...
<template>
  <div class="space-y-3">
    <CredentialsKeyOverrideToggle v-model:open="keysOpen" :override-mode="overrideMode" />
    ...
    <template v-if="keysOpen">
      <template v-if="provider === 'gcp'">
        ...
        <div v-if="gcpAuthMode === 'sa'" class="space-y-2">
          <label class="block space-y-2">
            <span class="lp-label">
              {{ t('credentials.fields.gcpSaKeyJson') }}
              <span v-if="overrideMode" class="font-normal text-[var(--lp-muted)]">
                ({{ t('common.optional') }})
              </span>
            </span>
            <textarea
          ...
          >
            {{ helpOpen ? t('credentials.fields.hideSaHelp') : t('credentials.fields.showSaHelp') }}
          </button>
          <div
            v-if="helpOpen"
            class="rounded-lg border border-[var(--lp-line)] bg-[var(--lp-panel-2)]/60 px-3 py-2 text-xs text-[var(--lp-muted)] space-y-1.5"
          >
      ...
      <template v-else-if="provider === 'aws'">
        ...
        <div v-if="awsAuthMode === 'keys'" class="grid gap-3 sm:grid-cols-2">
          <label class="block space-y-2">
            <span class="lp-label">
              {{ t('credentials.fields.accessKeyId') }}
              <span v-if="overrideMode" class="font-normal text-[var(--lp-muted)]"> ({{ t('common.optional') }})</span>
            </span>
            <input v-model="credentials.aws_access_key_id" class="lp-input" autocomplete="off">
          </label>
          <label class="block space-y-2">
            <span class="lp-label">
              {{ t('credentials.fields.secretAccessKey') }}
              <span v-if="overrideMode" class="font-normal text-[var(--lp-muted)]"> ({{ t('common.optional') }})</span>
            </span>
            <input
      ...
      <template v-else-if="provider === 'azure'">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          <label class="block space-y-2">
            <span class="lp-label">
              {{ t('credentials.fields.clientSecret') }}
              <span v-if="overrideMode" class="font-normal text-[var(--lp-muted)]"> ({{ t('common.optional') }})</span>
            </span>
            <input
          ...
          <label class="block space-y-2">
            <span class="lp-label">
              {{ t('credentials.fields.subscriptionId') }}
              <span v-if="overrideMode" class="font-normal text-[var(--lp-muted)]"> ({{ t('common.optional') }})</span>
            </span>
