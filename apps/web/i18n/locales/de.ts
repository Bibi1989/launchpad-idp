export default {
  "brand": {
    "name": "Launchpad",
    "tagline": "IDP",
    "product": "Launchpad IDP"
  },
  "meta": {
    "description": "Internes Entwicklerportal für ephemere Umgebungsbereitstellung"
  },
  "common": {
    "loading": "Laden…",
    "cancel": "Abbrechen",
    "close": "Schließen",
    "save": "Speichern",
    "delete": "Löschen",
    "confirm": "Bestätigen",
    "back": "Zurück",
    "next": "Weiter",
    "search": "Suchen",
    "refresh": "Aktualisieren",
    "open": "Öffnen",
    "download": "Herunterladen",
    "docs": "Dokumentation",
    "advanced": "Erweitert",
    "configured": "Konfiguriert",
    "notSet": "Nicht gesetzt",
    "viewAll": "Alle anzeigen →",
    "manage": "Verwalten",
    "theme": "Design",
    "themeDark": "Dunkel",
    "themeLight": "Hell",
    "language": "Sprache",
    "languageDe": "Deutsch",
    "languageEn": "English",
    "languageSystem": "System",
    "toggleTheme": "Design umschalten",
    "toggleLanguage": "Sprache ändern",
    "yes": "Ja",
    "no": "Nein",
    "working": "Wird ausgeführt…",
    "retry": "Erneut versuchen",
    "destroy": "Zerstören",
    "pause": "Pausieren",
    "resume": "Fortsetzen",
    "copy": "Kopieren",
    "copied": "Kopiert",
    "details": "Details",
    "logs": "Logs",
    "create": "Erstellen",
    "edit": "Bearbeiten",
    "remove": "Entfernen",
    "send": "Senden",
    "continue": "Weiter",
    "clear": "Leeren",
    "clearAll": "Alles leeren",
    "dismiss": "Schließen",
    "confirmDestroy": "Ja, zerstören",
    "confirmDelete": "Ja, löschen",
    "confirmRemove": "Ja, entfernen",
    "gitProvider": "Git-Anbieter",
    "optional": "optional",
    "required": "erforderlich",
    "name": "Name",
    "email": "E-Mail",
    "role": "Rolle",
    "status": "Status",
    "actions": "Aktionen",
    "saving": "Speichern…",
    "deleting": "Löschen…",
    "loadingEllipsis": "Laden…",
    "failed": "Etwas ist schiefgelaufen",
    "none": "Keine",
    "hours": "Stunden",
    "minutes": "Minuten",
    "branch": "Branch",
    "repository": "Repository",
    "private": "Privat",
    "public": "Öffentlich",
    "connected": "Verbunden",
    "disconnect": "Trennen",
    "readDocs": "Dokumentation lesen",
    "openApp": "App öffnen",
    "view": "Anzeigen",
    "apply": "Anwenden"
  },
  "gitBranch": {
    "default": "Standard",
    "selectRepoFirst": "Repository wählen, um Branches zu laden.",
    "createNew": "+ Neuen Branch erstellen…",
    "newPlaceholder": "feature/mein-branch",
    "createHint": "Der neue Branch wird beim Push vom Standard-Branch erstellt.",
    "loadFailed": "Branches konnten nicht geladen werden"
  },
  "nav": {
    "home": "Start",
    "environments": "Umgebungen",
    "launch": "Launch",
    "catalog": "Katalog",
    "workspaces": "Workspaces",
    "projects": "Projekte",
    "provision": "Provisionierung",
    "cloudPlugins": "Cloud-Plugins",
    "hybrid": "Fleet",
    "integrations": "Integrationen",
    "organization": "Organisation",
    "settings": "Einstellungen",
    "docs": "Doku",
    "groups": {
      "deploy": "Bereitstellen",
      "build": "Erstellen"
    }
  },
  "cloudPlugins": {
    "title": "Cloud-Plugins",
    "subtitle": "Cloud verbinden, API-Schluessel oder Token speichern und die Provisionierungs- und Konfigurationswerkzeuge waehlen.",
    "subtitleServices": "Deploy-Target-Plugins durchsuchen. Workspace erstellen (Provision) ist der Hauptweg; Advanced unten nur zum Vorschauen oder Dateien in einen bestehenden Workspace schreiben.",
    "serviceLabel": "Dienst",
    "credsConnected": "Zugangsdaten sind in den Einstellungen gesetzt.",
    "credsMissing": "Noch keine Zugangsdaten - Schluessel dieser Cloud in den Einstellungen hinzufuegen.",
    "manageInSettings": "In Einstellungen verwalten",
    "addPluginTitle": "Plugin registrieren (deklaratives Manifest)",
    "addPluginHint": "Manifest-JSON einfuegen: Metadaten (id, label, Zugangsdaten-Felder, Regionen, Groessen) + ein Runner (terraform / pulumi / ansible), der auf dein IaC-Bundle zeigt. Kein Code laeuft im Control Plane.",
    "registerPlugin": "Plugin registrieren",
    "pluginRegistered": "Plugin registriert",
    "manifestInvalidJson": "Manifest ist kein gueltiges JSON",
    "pluginBadge": "Plugin",
    "removePlugin": "Plugin entfernen",
    "bundleHint": "IaC-Bundle dieses Plugins (Terraform / Pulumi / Ansible) als .tar.gz oder .zip hochladen.",
    "uploadBundle": "Bundle hochladen",
    "uploading": "Wird hochgeladen...",
    "bundleUploaded": "Bundle hochgeladen ({count} Dateien)",
    "loading": "Anbieter werden geladen...",
    "connected": "Verbunden",
    "credentials": "Zugangsdaten",
    "credentialsHint": "Schluessel werden verschluesselt gespeichert und nicht erneut angezeigt. Feld leer lassen, um den gespeicherten Wert zu behalten.",
    "optional": "optional",
    "set": "gesetzt",
    "leaveBlankKeep": "Leer lassen, um gespeicherten Wert zu behalten",
    "reveal": "Anzeigen",
    "hide": "Verbergen",
    "save": "Speichern",
    "saving": "Speichern...",
    "saved": "Gespeichert",
    "validate": "Pruefen",
    "validating": "Pruefen...",
    "valid": "Zugangsdaten sind gueltig",
    "invalid": "Zugangsdaten abgelehnt",
    "disconnect": "Trennen",
    "docs": "Anbieter-Doku",
    "region": "Standardregion",
    "size": "Standardgroesse",
    "provisioningTool": "Provisionierungswerkzeug",
    "provisioningToolHint": "LaunchProvision ist Standard. Terraform, OpenTofu und Pulumi funktionieren mit jeder Cloud; native Werkzeuge gelten nur fuer ihre eigene Cloud.",
    "configTool": "Konfigurationswerkzeug",
    "configToolHint": "LaunchConfig ist der Standard. Es konfiguriert die Instanz, damit die App laufen kann. Auf Ansible wechseln oder ein Puppet/Chef-Plugin registrieren, um es zu ersetzen.",
    "advancedTitle": "Erweitert: Vorschau / Dateien schreiben",
    "advancedSubtitle": "Optional - nicht zum Deploy noetig",
    "advancedBlurb": "Provisionierungsdateien in einem bestehenden Workspace ansehen oder kopieren. Fuer neue Infrastruktur Provision (Workspace erstellen) nutzen - dort ist LaunchProvision Standard.",
    "registerConfigPlugin": "Config-Plugin registrieren zum Aktivieren",
    "servicesTitle": "Dienste (nach Ausfuehrungsart)",
    "artifactTitle": "Provisionierungsartefakt",
    "artifactHint": "Erzeugte Dateien ansehen und optional in ein Workspace-Verzeichnis schreiben. Die meisten Nutzer koennen dies ueberspringen und Provision nutzen.",
    "image": "Container-Image",
    "port": "App-Port",
    "targetWorkspace": "Ziel-Workspace",
    "chooseWorkspace": "Workspace waehlen...",
    "preview": "Vorschau",
    "previewing": "Wird erzeugt...",
    "scaffold": "In Workspace schreiben",
    "scaffolding": "Wird geschrieben...",
    "scaffoldDone": "{count} Datei(en) in den Workspace geschrieben",
    "openRegister": "Neues Cloud-Plugin registrieren",
    "editPlugin": "Plugin bearbeiten",
    "ownerUser": "Du",
    "ownerOrg": "Organisation",
    "published": "Oeffentlich",
    "registerModal": {
      "title": "Neues Cloud-Plugin registrieren",
      "editTitle": "Cloud-Plugin bearbeiten",
      "subtitle": "Feste Identitaetsfelder als Formular. Zugangsdaten- und Deploy-Config als JSON Schema (JSON oder YAML).",
      "boilerplate": "Boilerplate laden",
      "existing": "Aktuelles Plugin",
      "tabIdentity": "Basisdaten",
      "tabRunner": "Runner",
      "tabSchemas": "Schemas",
      "tabPreview": "Vorschau",
      "id": "Plugin-ID",
      "idPlaceholder": "digitalocean-droplet",
      "label": "Anzeigename",
      "labelPlaceholder": "DigitalOcean Droplets",
      "version": "Version",
      "category": "Kategorie",
      "description": "Beschreibung",
      "icon": "Icon",
      "iconSearch": "Cloud-Icons suchen...",
      "owner": "Besitzer",
      "ownerUser": "Du (persoenlich)",
      "ownerOrg": "Organisation",
      "visibility": "Sichtbarkeit",
      "publish": "Fuer alle Benutzer veroeffentlichen",
      "ownerHint": "Persoenliche Plugins stehen beim Erstellen eines Workspace, beim Provisionieren und beim Launch zur Verfuegung.",
      "runnerType": "Runner-Typ",
      "bundlePath": "Bundle-Pfad",
      "entrypoint": "Einstiegspunkt",
      "serviceType": "Diensttyp",
      "capabilities": "Faehigkeiten",
      "supportsTtl": "TTL unterstuetzt",
      "supportsCustomDns": "Eigenes DNS unterstuetzt",
      "supportsEphemeralDb": "Ephemere Datenbank unterstuetzt",
      "previewHint": "Nur-lesen Manifest.json aus Formularfeldern und Schema-Editoren.",
      "validate": "Manifest pruefen",
      "validating": "Wird geprueft...",
      "validateOk": "Manifest ist gueltig",
      "validateFailed": "Manifest-Pruefung fehlgeschlagen",
      "save": "Speichern und Plugin registrieren",
      "saveFailed": "Plugin konnte nicht registriert werden",
      "lineError": "Zeile {line}: {message}",
      "aiPrompt": "KI ein Plugin erzeugen lassen",
      "aiPromptPlaceholder": "z.B. GKE-Cluster, der den GCP-Service-Account aus den Einstellungen nutzt",
      "aiHint": "Gemini fuellt das Manifest, wenn konfiguriert; sonst ein Keyword-Heuristik.",
      "aiGenerate": "Mit KI erzeugen",
      "aiGenerating": "Wird erzeugt...",
      "aiDone": "Plugin-Entwurf bereit",
      "aiFailed": "Plugin konnte nicht erzeugt werden",
      "aiPromptShort": "Beschreibe das Plugin etwas genauer (mindestens 8 Zeichen).",
      "aiFromGemini": "Aus Gemini gefuellt. Felder vor dem Speichern pruefen.",
      "aiFromHeuristic": "Aus lokaler Heuristik gefuellt. Felder vor dem Speichern pruefen.",
      "aiGenerateSchemas": "Schemas mit KI erzeugen",
      "aiSchemasHint": "credentialsSchema und deploymentConfigSchema aus Parent-Cloud und Service-Typ entwerfen. Optionale Notizen verfeinern die Felder.",
      "aiSchemasPlaceholder": "z. B. GKE mit Artifact Registry, ausser Images kommen von GHCR, Secret Manager fuer Secrets",
      "aiSchemasService": "Nutzt {cloud} / {service}",
      "aiSchemasNoParent": "Parent-Cloud unter Basisdaten setzen fuer Settings-Keys, oder den Dienst in den Notizen beschreiben.",
      "aiSchemasDone": "Schemas entworfen",
      "aiSchemasFailed": "Schemas konnten nicht erzeugt werden",
      "showMore": "Weitere optionale Felder",
      "hideMore": "Optionale Felder ausblenden",
      "parentCloud": "Uebergeordnete Cloud (Settings-Keys)",
      "parentCloudNone": "Keine (plugin-eigene Keys)",
      "parentCloudHint": "GKE, EKS, AKS und Cloudflare nutzen die passenden Keys aus den Einstellungen.",
      "author": "Autor",
      "license": "Lizenz",
      "keywords": "Schlagwoerter",
      "keywordsPlaceholder": "gke, kubernetes, gcp",
      "docsUrl": "Docs-URL",
      "homepage": "Homepage",
      "convertJson": "Nach JSON konvertieren",
      "convertYaml": "Nach YAML konvertieren"
    }
  },
  "cloudDeployGrid": {
    "label": "Cloud-Ziel",
    "manage": "Cloud-Plugins"
  },
  "cloudPluginPicker": {
    "label": "Cloud-Ziel (Plugin)",
    "manage": "Cloud-Plugins",
    "none": "Keins (Standard-Runtime verwenden)",
    "groupYours": "Deine Plugins",
    "groupOrg": "Organisation",
    "groupPublic": "Veroeffentlicht",
    "groupBuiltin": "Eingebaute Clouds",
    "notConnected": "Noch nicht verbunden - Schluessel dieser Cloud in den Einstellungen hinzufuegen.",
    "service": "Dienst",
    "region": "Region",
    "size": "Groesse"
  },
  "pluginCreds": {
    "title": "Cloud-Plugin-Anbieter",
    "subtitle": "API-Schluessel/Token fuer die Plugin-Clouds (Hetzner, DigitalOcean, Railway). Verschluesselt gespeichert; von Cloud-Plugins genutzt.",
    "optional": "optional",
    "set": "gesetzt",
    "leaveBlankKeep": "Leer lassen, um gespeicherten Wert zu behalten",
    "reveal": "Anzeigen",
    "hide": "Verbergen",
    "save": "Speichern",
    "saving": "Speichern...",
    "saved": "Gespeichert",
    "validate": "Pruefen",
    "validating": "Pruefen...",
    "valid": "Zugangsdaten sind gueltig",
    "invalid": "Zugangsdaten abgelehnt",
    "disconnect": "Trennen"
  },
  "shell": {
    "signedIn": "Angemeldet",
    "organization": "Organisation",
    "newEnvironment": "Neue Umgebung",
    "terminal": "Terminal",
    "logout": "Abmelden",
    "openNav": "Navigation öffnen",
    "closeNav": "Navigation schließen",
    "searchPlaceholder": "Seiten, Umgebungen, Workspaces suchen…",
    "searchAria": "Suche",
    "noMatches": "Keine Treffer für „{query}“",
    "launchPreview": "Preview starten",
    "groups": {
      "Pages": "Seiten",
      "Environments": "Umgebungen",
      "Workspaces": "Workspaces",
      "Services": "Services"
    },
    "initializing": "Workspace wird initialisiert",
    "filterMenu": "Menü filtern…",
    "filterNav": "Navigation filtern",
    "noMenuMatches": "Keine Menütreffer",
    "apiDocs": "API-Doks",
    "toggleTerminal": "Terminal ein-/ausblenden",
    "closeTerminal": "Terminal schließen",
    "terminalSandbox": "Terminal-Sandbox",
    "liveSessionAttached": "Live-Sitzung verbunden",
    "openProvisionToAttach": "Provision → Erzeugen öffnen, um eine Live-Sitzung zu verbinden",
    "liveSessionOnPage": "Live-Sitzung ist an das Terminal-Panel dieser Seite gebunden.",
    "closeDrawer": "Drawer schließen",
    "noSandboxSession": "Keine aktive Sandbox-Sitzung.",
    "goToProvision": "Zu Provision",
    "launchEnvironment": "Umgebung starten",
    "launchEnvironmentBlurb": "Isolierten Namespace mit TTL-Governance bereitstellen."
  },
  "landing": {
    "eyebrow": "Internes Entwicklerportal",
    "heroTitle": "Launchpad",
    "heroHeadline": "Gesteuerte Umgebungen über jede Cloud ausrollen.",
    "heroBlurb": "Eine Control Plane für ephemere Previews, Multi-Cloud-IaC, Kubernetes-Workloads und Hybrid-Hosts - ohne das Portal zu verlassen, dem Ihr Platform-Team bereits vertraut.",
    "heroTrust": "Für Platform-, SRE- und Application-Teams",
    "getStarted": "Loslegen",
    "readDocs": "Dokumentation lesen",
    "docs": "Doku",
    "openHub": "Hub öffnen",
    "signIn": "Anmelden",
    "gettingStarted": "Erste Schritte",
    "apiReference": "API-Referenz",
    "nav": {
      "product": "Produkt",
      "platforms": "Plattformen",
      "security": "Sicherheit",
      "demo": "Demo"
    },
    "demo": {
      "ariaLabel": "Launchpad Produktdemo-Video",
      "caption": "End-to-end Walkthrough: Launch, Status und Provisionierung"
    },
    "platforms": {
      "eyebrow": "Passt zum Stack, den Sie bereits betreiben",
      "gcp": "Google Cloud",
      "aws": "AWS",
      "azure": "Azure",
      "cloudflare": "Cloudflare",
      "kubernetes": "Kubernetes",
      "github": "GitHub",
      "gitlab": "GitLab",
      "terraform": "Terraform / Pulumi"
    },
    "capabilities": {
      "eyebrow": "Fähigkeiten",
      "title": "Alles, was Ihr Portal braucht, um Code in Runtime zu bringen",
      "blurb": "Von Katalogvorlagen bis zu Live-Previews und Cloud-Apply hält Launchpad Provisionierung, Policy und Day-2-Ops in einem gesteuerten Workflow.",
      "previews": {
        "title": "Ephemere Previews",
        "blurb": "Gesteuerte App-Umgebungen aus Repo oder Katalogvorlage starten und per TTL wieder abbauen."
      },
      "iac": {
        "title": "Multi-Cloud-IaC",
        "blurb": "Terraform oder Pulumi für GCP, AWS, Azure, Cloudflare oder lokale Sandbox erzeugen und aus der Sandbox anwenden."
      },
      "k8s": {
        "title": "Kubernetes-Workloads",
        "blurb": "Gehärte Manifeste bearbeiten, auf den Cluster anwenden und Pods mit Describe, Logs und Exec betreiben."
      },
      "catalog": {
        "title": "Service-Katalog",
        "blurb": "Golden Paths veröffentlichen, damit Teams den richtigen Stack beim ersten Mal starten - mit Org-Standards."
      },
      "hybrid": {
        "title": "Hybrid-Cloud-Agenten",
        "blurb": "Private Netze und On-Prem-Hosts über ausgehende Agenten erreichen - ohne eingehende Firewall-Ausnahmen."
      },
      "governance": {
        "title": "Org-Governance",
        "blurb": "SSO, Rollen, Credential-Vaults und Audit-Trails machen Self-Service für Platform Owner sicher."
      }
    },
    "steps": {
      "eyebrow": "So funktioniert es",
      "title": "Vom Commit zur laufenden Umgebung in drei Schritten",
      "blurb": "Launchpad erledigt Buchhaltung in der API und schwere Arbeit in Workern, damit Browser nie auf Terraform oder Image-Builds warten.",
      "connect": {
        "title": "Quellen verbinden",
        "blurb": "GitHub oder GitLab anbinden, Cloud-Credentials einmal hinterlegen und Katalogvorlage oder Workspace wählen."
      },
      "launch": {
        "title": "Starten oder provisionieren",
        "blurb": "Preview starten, IaC erzeugen oder Manifeste anwenden. Celery-Worker klonen, bauen und applyen, während Sie den Status live sehen."
      },
      "operate": {
        "title": "Mit Leitplanken betreiben",
        "blurb": "Logs prüfen, in Pods execen, bei Push neu bauen und per TTL abbauen - alles unter Org-Policy."
      }
    },
    "security": {
      "eyebrow": "Sicherheit & Governance",
      "title": "Self-Service, ohne die Schlüssel abzugeben",
      "blurb": "Platform-Teams behalten Kontrolle über Identität, Secrets und Blast Radius, während Entwickler innerhalb klarer Leitplanken schneller werden.",
      "cta": "Architektur-Dokumentation öffnen",
      "points": {
        "sso": "OIDC-SSO mit Org-Rollenmapping für Operatoren und Entwickler",
        "vault": "Verschlüsselter Credential-Vault für GCP, AWS, Azure und Cloudflare",
        "sandbox": "Sandboxed Apply-Pfade und keyless Workload Identity wo konfiguriert",
        "audit": "Environment-Lifecycle und Rebuilds lückenlos nachvollziehbar"
      }
    },
    "stats": {
      "clouds": {
        "value": "4+",
        "label": "Cloud-Ziele in einem Portal"
      },
      "engines": {
        "value": "3",
        "label": "IaC-Engines (Terraform, OpenTofu, Pulumi)"
      },
      "runtime": {
        "value": "K8s+",
        "label": "Previews, Compose und Instance-Modi"
      },
      "gitops": {
        "value": "Push",
        "label": "Webhook-Rebuilds für Live-Branches"
      }
    },
    "finalCta": {
      "title": "Bereit für Ihre nächste Umgebung?",
      "blurb": "Hub öffnen, Repo verbinden und gesteuerte Preview starten - oder mit der Doku Ihre erste Org verdrahten."
    },
    "footer": {
      "blurb": "Das interne Entwicklerportal für gesteuerte Previews, Multi-Cloud-Infrastruktur und Hybrid-Betrieb.",
      "product": "Produkt",
      "resources": "Ressourcen",
      "access": "Zugang",
      "rights": "Für Platform-Teams gebaut"
    },
    "features": {
      "previews": {
        "title": "Ephemere Previews",
        "blurb": "Gesteuerte App-Umgebungen aus Repo oder Katalogvorlage starten und per TTL wieder abbauen."
      },
      "iac": {
        "title": "Multi-Cloud-IaC",
        "blurb": "Terraform oder Pulumi für GCP, AWS, Azure, Cloudflare oder lokale Sandbox erzeugen und aus der Sandbox anwenden."
      },
      "k8s": {
        "title": "Kubernetes-Workloads",
        "blurb": "Gehärte Manifeste bearbeiten, auf den Cluster anwenden und Pods mit Describe, Logs und Exec betreiben."
      }
    },
    "graphic": {
      "title": "So fügt sich Launchpad zusammen",
      "description": "Entwickler und Git-Repositories verbinden sich mit der Launchpad-Control-Plane, die Previews, Multi-Cloud-Infrastruktur und Hybrid-Agenten startet.",
      "sourceLabel": "SIE",
      "sourceTitle": "Entwickler & Orgs",
      "gitLabel": "QUELLE",
      "gitTitle": "GitHub / GitLab",
      "hubLabel": "CONTROL PLANE",
      "hubSubtitle": "API · Worker · Governance",
      "targetPreview": "Previews",
      "targetPreviewHint": "K8s · TTL",
      "targetCloud": "Cloud-IaC",
      "targetCloudHint": "GCP · AWS · Azure",
      "targetHybrid": "Hybrid",
      "targetHybridHint": "Host-Agent",
      "stepLaunch": "Starten",
      "stepProvision": "Provisionieren",
      "stepPreview": "Preview",
      "stepOperate": "Betreiben"
    }
  },
  "auth": {
    "login": "Anmelden",
    "register": "Registrieren",
    "email": "E-Mail",
    "password": "Passwort",
    "showPassword": "Passwort anzeigen",
    "hidePassword": "Passwort verbergen",
    "displayName": "Anzeigename",
    "submitLogin": "Anmelden",
    "submitRegister": "Konto erstellen",
    "devLogin": "Dev-Login",
    "ssoLogin": "Mit SSO anmelden",
    "subtitle": "Melden Sie sich an, um gesteuerte Umgebungen zu provisionieren",
    "invalidForm": "Ungültiges Formular",
    "loginFailed": "Anmeldung fehlgeschlagen",
    "registerFailed": "Registrierung fehlgeschlagen",
    "devLoginFailed": "Dev-Login fehlgeschlagen",
    "ssoFailed": "SSO-Anmeldung fehlgeschlagen",
    "callbackTitle": "Anmeldung wird abgeschlossen…",
    "callbackBlurb": "OIDC-Autorisierungscode wird ausgetauscht",
    "callbackFailed": "OIDC-Callback fehlgeschlagen",
    "backToLogin": "Zurück zur Anmeldung",
    "missingOidc": "OIDC-Code oder State fehlt",
    "oidcStateMismatch": "OIDC-State stimmt nicht - Anmeldung neu starten"
  },
  "home": {
    "eyebrow": "Launchpad",
    "welcome": "Willkommen",
    "welcomeNamed": "Willkommen, {name}",
    "blurb": "Gesteuerte ephemere Umgebungen und Multi-Cloud-Infrastruktur aus einem Portal. Starten Sie eine Preview, provisionieren Sie einen Stack oder verwalten Sie Cloud-Zugangsdaten.",
    "activeEnvironments": "Aktive Umgebungen",
    "workspaces": "Workspaces",
    "cloudKeys": "Cloud-Schlüssel des Kontos",
    "openWorkspaces": "Workspaces öffnen →",
    "manageCredentials": "Zugangsdaten verwalten →",
    "addKeys": "Schlüssel unter Einstellungen hinzufügen",
    "quickActions": "Schnellaktionen",
    "observability": {
      "title": "Gesundheit & Metriken",
      "blurb": "Live-HTTP-Health-Pings und Namespace-CPU/Speicher für aktive Environments.",
      "refresh": "Aktualisieren",
      "healthy": "Gesunde Pings",
      "unhealthy": "Fehlgeschlagene Pings",
      "unknown": "Keine Preview-URL",
      "empty": "Noch keine aktiven Environments. Starte eine Preview, um Gesundheit und Metriken zu sehen.",
      "loadFailed": "Observability-Daten konnten nicht geladen werden",
      "cpu": "CPU",
      "memory": "Speicher",
      "pingOk": "Ping OK",
      "pingFail": "Ping fehlgeschlagen",
      "pingNone": "Keine URL"
    },
    "actions": {
      "launch": {
        "title": "Preview starten",
        "blurb": "Ephemere App-Umgebung aus Vorlage oder Git-Repo starten."
      },
      "provision": {
        "title": "Infra provisionieren",
        "blurb": "Terraform oder Pulumi für GCP, AWS, Azure oder kind erzeugen."
      },
      "catalog": {
        "title": "Katalog durchsuchen",
        "blurb": "Golden-Path-Vorlagen und Workspaces, die du im Katalog markierst."
      },
      "credentials": {
        "title": "Cloud-Zugangsdaten",
        "blurb": "GCP-, AWS-, Azure- oder Cloudflare-Schlüssel für Ihr Konto speichern."
      }
    }
  },
  "integrations": {
    "title": "Integrationen",
    "github": "GitHub",
    "gitlab": "GitLab",
    "githubBlurb": "Autorisieren Sie Launchpad, Repositories zu erstellen und CI-Workflows in Ihrem Namen zu pushen. Sie werden zu GitHub weitergeleitet, um die App zu installieren, und kehren dann hierher zurück.",
    "connectGithub": "GitHub verbinden",
    "connectGithubBlurb": "Autorisieren Sie Launchpad auf GitHub, um Repositories zu erstellen und Deploy-Workflows zu pushen – wie bei Vercel.",
    "setupGuide": "Setup-Anleitung",
    "needAppId": "Setzen Sie GITHUB_APP_ID und einen privaten Schlüssel an der API, um GitHub Connect zu aktivieren",
    "sourceControl": "Quellcodeverwaltung",
    "sourceControlBlurb": "GitHub oder GitLab verbinden, damit Launchpad Repos anlegen und Workflows pushen kann.",
    "preferGithub": "Lieber GitHub?",
    "preferGitlab": "Lieber GitLab?",
    "checkingGithub": "GitHub-Verbindung wird geprüft…",
    "connectedGithub": "Mit GitHub verbunden",
    "live": "Live",
    "createRepo": "Neues Repository erstellen",
    "selectRepo": "Repository auswählen",
    "selectAccount": "Konto auswählen",
    "justAuthorized": "gerade autorisiert",
    "personalWarning": "Persönliche GitHub-Konten können über App-Installationstokens keine neuen Repos erstellen. Bevorzuge Import Git Repository, oder lege zuerst ein leeres Repo auf GitHub an und nutze Create mit demselben Namen. Org-Installationen können Repos direkt erstellen.",
    "manageInstalls": "Installationen verwalten",
    "githubAccount": "GitHub-Konto",
    "using": "Verwendet",
    "searchRepos": "Persönliche & Org-Repos suchen…",
    "fetchingRepos": "Repositories werden geladen…",
    "noRepos": "Keine Repositories gefunden",
    "connectGitlab": "GitLab verbinden",
    "connectGitlabBlurb": "Launchpad mit GitLab-OAuth oder Personal Access Token autorisieren.",
    "disconnectGitlab": "GitLab trennen",
    "nextStepsGithub": "Als Nächstes: Installation und Repository wählen, dann Launch oder Provision.",
    "nextStepsGitlab": "Als Nächstes: Projekt wählen, dann Launch oder Provision.",
    "githubCardBlurb": "GitHub-App-Installation für Repo-Erstellung und Workflow-Push.",
    "gitlabCardBlurb": "OAuth oder Personal Access Token für Projekt-Erstellung und gespiegelten Datei-Push.",
    "hubTitle": "Integrationen",
    "hubBlurb": "Quellcodeverwaltung, Chat und Issue-Tracker für Ihre Organisation verbinden.",
    "collaboration": "Zusammenarbeit",
    "slack": "Slack",
    "jira": "Jira",
    "slackCardBlurb": "Incoming-Webhook-Benachrichtigungen für Ready-, Failed-, TTL- und Kosten-Events.",
    "jiraCardBlurb": "Jira-Tickets erstellen oder verknüpfen, wenn Previews fehlschlagen.",
    "slackPageBlurb": "Org-weite Slack-Alerts bei Statusänderungen ephemerer Environments.",
    "jiraPageBlurb": "Jira Cloud verbinden, damit fehlgeschlagene Previews dauerhafte Tickets öffnen können.",
    "connectSlack": "Slack verbinden",
    "connectSlackBlurb": "Slack Incoming Webhook URL einfügen und Lifecycle-Events wählen.",
    "connectJira": "Jira Cloud verbinden",
    "connectJiraBlurb": "Atlassian-E-Mail und API-Token für Ihre Jira-Cloud-Site verwenden.",
    "connected": "Verbunden",
    "loading": "Integration wird geladen…",
    "selectOrg": "Organisation wählen, um Integrationen zu verwalten.",
    "adminOnly": "Nur Org-Owner und Admins können diese Integration ändern.",
    "slackWebhookUrl": "Incoming-Webhook-URL",
    "slackWebhookHint": "Incoming Webhook in Slack erstellen und die https://hooks.slack.com URL hier einfügen.",
    "webhookConfigured": "Webhook gespeichert (neue URL zum Ersetzen eingeben)",
    "slackEvents": "Benachrichtigen bei",
    "eventReady": "Preview bereit",
    "eventFailed": "Preview fehlgeschlagen",
    "eventTtl": "TTL-Warnung / abgelaufen",
    "eventCost": "Soft Cost Cap",
    "disconnectSlack": "Slack trennen",
    "slackSaved": "Slack aktualisiert",
    "slackSavedBody": "Webhook und Event-Einstellungen für diese Organisation gespeichert.",
    "slackDisconnected": "Slack getrennt",
    "slackDisconnectedBody": "Incoming Webhook für diese Organisation entfernt.",
    "slackLoadFailed": "Slack-Integration konnte nicht geladen werden",
    "slackSaveFailed": "Slack-Integration konnte nicht gespeichert werden",
    "jiraSiteUrl": "Jira-Site-URL",
    "jiraEmail": "Atlassian-E-Mail",
    "jiraApiToken": "API-Token",
    "jiraProjectKey": "Projektschlüssel",
    "jiraIssueType": "Issuetyp",
    "jiraAutoCreate": "Bei Preview-Fehler automatisch ein Jira-Ticket erstellen",
    "tokenConfigured": "Token gespeichert (neues Token zum Ersetzen eingeben)",
    "disconnectJira": "Jira trennen",
    "jiraSaved": "Jira aktualisiert",
    "jiraSavedBody": "Jira-Cloud-Zugangsdaten für diese Organisation gespeichert.",
    "jiraDisconnected": "Jira getrennt",
    "jiraDisconnectedBody": "Jira-Zugangsdaten für diese Organisation entfernt.",
    "jiraLoadFailed": "Jira-Integration konnte nicht geladen werden",
    "jiraSaveFailed": "Jira-Integration konnte nicht gespeichert werden",
    "backToHub": "Zurück zu Integrationen",
    "createJiraIssue": "Jira-Ticket erstellen",
    "openJiraIssue": "Jira öffnen",
    "linkingJira": "Jira-Ticket wird erstellt…",
    "jiraIssueCreated": "Jira-Ticket erstellt",
    "jiraIssueLinked": "Jira-Ticket verknüpft",
    "jiraIssueFailed": "Jira-Ticket konnte nicht erstellt werden",
    "jiraIssueLabel": "Jira",
    "nextSteps": "Nächste Schritte",
    "githubStep1": "Cloud-Workspace erstellen unter",
    "githubStep2": "Im GitHub-Schritt Installation wählen und Repo + Workflow erstellen.",
    "githubStep3": "Oder bestehenden Workspace öffnen unter",
    "gitlabStep1": "Einen",
    "gitlabStep2": "Nur im Workspace-Baum sichtbare Dateien werden committed (kein Pfad-Mapping).",
    "gitlabPublish": "Veröffentlichen → GitLab.",
    "needCloudCreds": "Zuerst GCP/AWS-Zugangsdaten nötig?",
    "gitlabPageBlurb": "GitLab per OAuth oder Personal Access Token verbinden, um Projekte zu erstellen und Workspace-Dateien mit demselben Verzeichnislayout wie in Launchpad zu pushen.",
    "addAccount": "Konto hinzufügen",
    "reconnect": "Erneut verbinden",
    "newRepoName": "Neuer Repository-Name",
    "importGitRepo": "Git-Repository importieren",
    "noReposVisible": "Keine Repositories für diese Installation sichtbar. Neues Repo erstellen oder der App mehr Repositories auf GitHub freigeben.",
    "searchSelectRepo": "Repository suchen & auswählen…",
    "noReposMatching": "Keine Repositories für \"{query}\" gefunden",
    "loadingReposPicker": "Repositories werden geladen…",
    "searchProjects": "GitLab-Projekte suchen…",
    "searchSelectProject": "Projekt suchen & auswählen…",
    "noProjectsMatching": "Keine Projekte gefunden für \"{query}\"",
    "loadingProjectsPicker": "Projekte werden geladen…",
    "fetchingProjects": "Projekte werden geladen…",
    "gitlabProjectsLoadFailed": "GitLab-Projekte konnten nicht geladen werden",
    "gitlabConnectedToast": "GitLab verbunden",
    "gitlabDisconnectedToast": "GitLab getrennt",
    "gitlabConnectFailed": "GitLab-Verbindung fehlgeschlagen",
    "gitlabDisconnectFailed": "GitLab-Trennung fehlgeschlagen",
    "gitlabStatusFailed": "GitLab-Status konnte nicht geladen werden",
    "gitlabPatFailed": "GitLab-Token konnte nicht verbunden werden",
    "gitlabOauthFailed": "GitLab-OAuth fehlgeschlagen",
    "connectOAuth": "Mit GitLab OAuth verbinden",
    "usePat": "Personal Access Token nutzen",
    "hidePatForm": "Token-Formular ausblenden",
    "gitlabBaseUrl": "GitLab-Basis-URL",
    "patToken": "Personal Access Token",
    "patScopes": "Benötigt api- und write_repository-Scopes.",
    "connecting": "Verbinden…",
    "saveToken": "Token speichern",
    "checkingGitlab": "GitLab-Verbindung wird geprüft…",
    "gitlabConnectBlurb": "Launchpad autorisieren, Projekte zu erstellen und dieselben Dateien wie im Workspace zu pushen.",
    "gitlabHint": "gitlab.com / Self-hosted"
  },
  "settings": {
    "title": "Einstellungen",
    "appearance": "Darstellung",
    "appearanceBlurb": "Design und Sprache für dieses Gerät festlegen.",
    "preferences": "Einstellungen",
    "profileBlurb": "Profil für {email}. Cloud-Schlüssel einmal speichern - Provision und Sandbox können sie nutzen, wenn Workspace-Felder leer sind.",
    "cloudCredentials": "Cloud-Zugangsdaten",
    "credentialsBlurb": "Schlüssel werden mit Fernet verschlüsselt gespeichert. Feld leer lassen, um den gespeicherten Wert zu behalten.",
    "clearAll": "Alles löschen",
    "clearing": "Löschen…",
    "clearProvider": "Gespeicherte Keys entfernen",
    "clearProviderNamed": "{provider}-Keys entfernen",
    "stored": "Gespeichert:",
    "configured": "konfiguriert",
    "replaceHint": "Neue Werte eingeben zum Ersetzen, oder gespeicherte Keys unten entfernen.",
    "removeKeysConfirm": "Gespeicherte {provider}-Keys von diesem Konto entfernen?",
    "saving": "Speichern…",
    "saveProvider": "{provider} speichern",
    "credentialsSaved": "Zugangsdaten gespeichert (verschlüsselt).",
    "clearedProvider": "{provider}-Keys entfernt.",
    "clearedAll": "Alle Cloud-Zugangsdaten des Kontos entfernt.",
    "credentialsBlurbExtended": "Schlüssel werden mit Fernet verschlüsselt gespeichert. Service-Account-JSON oder keyless WIF bevorzugen. Browser-Connect ist optional ohne Schlüssel.",
    "oauthConnectTitle": "Per Browser verbinden (optional)",
    "oauthConnectHint": "Optionaler Fallback wie gcloud auth login. Für geteilte Org-Cloud besser Service-Account-JSON unten einfügen.",
    "oauthConnect": "{provider} verbinden",
    "oauthConnecting": "Warte auf Browser…",
    "oauthConnected": "Cloud-Konto verbunden und gespeichert.",
    "oauthNeedGcpClient": "GCP_OAUTH_CLIENT_ID (Google Desktop-OAuth-Client) in der API setzen.",
    "oauthNeedAzureClient": "AZURE_OAUTH_CLIENT_ID (Entra Public/Native App) in der API setzen.",
    "awsStartUrl": "IAM Identity Center Start-URL",
    "awsRegion": "SSO-Region",
    "awsAccountIdOptional": "AWS-Konto-ID (optional)",
    "awsRoleNameOptional": "Permission Set / Rollenname (optional)",
    "azureTenantOptional": "Tenant-ID (optional, Standard common)",
    "azureSubscriptionOptional": "Subscription-ID (optional)",
    "alsoConfigure": "Auch konfigurieren",
    "alsoOrg": "Organisation - Mitglieder und SSO",
    "alsoIntegrations": "Integrationen - GitHub und GitLab",
    "localKubernetes": "Lokales Kubernetes",
    "localKubernetesBlurb": "Lokalen Cluster für Launch → Local und lokale Provision erstellen oder löschen (Standard: k3d/k3s, oder kind). Am besten den vom API konfigurierten Namen (KIND_CLUSTER_NAME) verwenden, damit Launches denselben Cluster treffen.",
    "localKubernetesName": "Cluster-Name",
    "localKubernetesNameHint": "DNS-Label (Kleinbuchstaben). Standard belassen, außer KIND_CLUSTER_NAME auf der API wird ebenfalls angepasst.",
    "localKubernetesCreate": "Cluster erstellen",
    "localKubernetesDelete": "Cluster löschen",
    "localKubernetesCreating": "Wird erstellt…",
    "localKubernetesDeleting": "Wird gelöscht…",
    "localKubernetesRefresh": "Status aktualisieren",
    "localKubernetesCreated": "Lokaler Cluster ist bereit.",
    "localKubernetesDeleted": "Lokaler Cluster gelöscht.",
    "localKubernetesConfirmDelete": "Lokalen Cluster '{name}' löschen? Laufende lokale Previews und Workloads darauf werden beendet.",
    "localKubernetesStatus": "Status",
    "localKubernetesEngine": "Engine",
    "localKubernetesContext": "kubectl-Kontext",
    "errors": {
      "load": "Zugangsdaten konnten nicht geladen werden",
      "save": "Speichern fehlgeschlagen",
      "clear": "Löschen fehlgeschlagen",
      "oauthFailed": "Browser-Connect fehlgeschlagen oder Zeitüberschreitung",
      "vaultUnreadable": "Gespeicherte Cloud-Schlüssel konnten nicht entschlüsselt werden (Verschlüsselungsschlüssel möglicherweise geändert). Bitte Schlüssel erneut einfügen und speichern.",
      "kindLoad": "Lokaler Cluster-Status konnte nicht geladen werden",
      "kindUp": "Lokaler Cluster konnte nicht erstellt werden",
      "kindDown": "Lokaler Cluster konnte nicht gelöscht werden"
    }
  },
  "pages": {
    "environments": {
      "title": "Umgebungen",
      "empty": "Noch keine Umgebungen.",
      "create": "Umgebung erstellen"
    },
    "launch": {
      "title": "Preview starten",
      "blurb": "Ephemere App-Umgebung aus Vorlage oder Repository starten."
    },
    "catalog": {
      "title": "Katalog",
      "blurb": "Golden-Path-Vorlagen und markierte Workspaces."
    },
    "workspaces": {
      "title": "Workspaces",
      "empty": "Noch keine Workspaces."
    },
    "provision": {
      "title": "Provisionierung",
      "blurb": "Infrastruktur als Code erzeugen und anwenden."
    },
    "organization": {
      "title": "Organisation",
      "blurb": "Mitglieder, Einladungen und SSO."
    },
    "docs": {
      "title": "Dokumentation",
      "blurb": "Launchpad nutzen und betreiben."
    }
  },
  "invite": {
    "title": "Organisationseinladung",
    "blurb": "Einladungstoken wird angenommen",
    "working": "Wird ausgeführt…",
    "failed": "Einladung konnte nicht angenommen werden",
    "accepted": "Einladung angenommen",
    "joinedAs": "Beigetreten als {role}. Weiterleitung…"
  },
  "import": {
    "redirecting": "Weiterleitung zu Workspaces…",
    "modalTitle": "Repository importieren",
    "modalBlurb": "GitHub- oder GitLab-Repository in einen Launchpad-Workspace klonen.",
    "searchRepo": "Repository suchen",
    "modalBlurbFull": "GitHub oder GitLab wählen, Stack analysieren, dann als Workspace speichern.",
    "loadingGithubAccounts": "GitHub-Konten werden geladen…",
    "noGithubInstalls": "Keine GitHub-App-Installationen gefunden.",
    "connectGithubOrPaste": "(persönlich und/oder Organisation) oder URL einfügen.",
    "connectedGitlabProject": "Verbundenes GitLab-Projekt",
    "selectProject": "Projekt auswählen…",
    "gitlabNotConnected": "GitLab ist nicht verbunden.",
    "gitlabPasteUrl": "oder öffentliche URL einfügen.",
    "noGitlabProjects": "Keine Projekte zurückgegeben. GitLab-URL unten einfügen.",
    "repoUrlGithub": "GitHub-Repository-URL",
    "repoUrlGitlab": "GitLab-Repository-URL",
    "workspaceName": "Workspace-Name",
    "cloningDetecting": "Klonen & Erkennen…",
    "analyzeRepo": "Repository analysieren",
    "multiRepoTitle": "Weitere Repositories",
    "multiRepoBlurb": "Mehrere Microservices in einen Workspace importieren. Oben ein Repo auswählen, dann hinzufügen; für jeden Dienst wiederholen.",
    "addRepo": "Repository hinzufügen",
    "confirmStack": "Erkannten Stack bestätigen",
    "startOver": "Neu starten",
    "savingWorkspace": "Workspace wird gespeichert…",
    "saveAsWorkspace": "Als Workspace speichern",
    "invalidGithubUrl": "Gültige GitHub-Repository-URL eingeben",
    "invalidGitlabUrl": "Gültige GitLab-Repository-URL eingeben",
    "selectGithubAccount": "GitHub-Konto (persönlich oder Organisation) vor dem Import auswählen",
    "importFailed": "Import fehlgeschlagen",
    "workspaceNameInvalid": "Workspace-Name muss kleingeschrieben sein, mit Buchstabe beginnen, 3-64 Zeichen",
    "saveWorkspaceFailed": "Workspace konnte nicht gespeichert werden",
    "runtimeMode": "Runtime",
    "runtimeModeHint": "Kubernetes importiert Manifeste. Compose und Running Instance importieren Docker-Artefakte und können IaC + CI/CD scaffolden.",
    "detectedKubernetes": "Kubernetes-Manifeste in diesem Repository erkannt.",
    "detectedCompose": "Docker-Compose-Datei in diesem Repository erkannt.",
    "runtimeModeOverrideHint": "Du kannst weiterhin Kubernetes, Docker Compose oder Running Instance für Scaffold und Provision wählen.",
    "enableIac": "IaC scaffolden (Terraform / Pulumi)",
    "enableCicd": "CI/CD scaffolden",
    "cicdPlatform": "CI-Plattform",
    "datastoresTitle": "Datenbanken & Cache",
    "datastoresBlurb": "In-Cluster für lokale Previews, oder externe URL (Neon, Supabase, RDS, Upstash, Railway, …).",
    "datastoreInCluster": "In-Cluster (Launchpad)",
    "datastoreExternal": "Externe URL",
    "datastoreSkip": "Überspringen",
    "datastoreInClusterHint": "Launchpad startet Postgres/Redis neben der App und setzt Verbindungsvariablen.",
    "datastoreInClusterAutoInjected": "Launchpad betreibt {kind} im Cluster und injiziert die Verbindungs-URL automatisch als Umgebungsvariable (z.B. DATABASE_URL / REDIS_URL). Keine Konfiguration nötig.",
    "datastoreExternalHint": "Empfohlen für shared/managed DBs. z.B. Neon/Supabase Postgres, Upstash Redis.",
    "connectionUrl": "Connection-URL",
    "useSuggestedUrl": "Vorschlag verwenden",
    "envTitle": "Umgebungsvariablen",
    "envBlurb": "Aus .env.example (oder .env.launchpad) je Service. Secrets bleiben leer. Launchpad injiziert sie in Preview-Pods, Docker und VMs. Beim Speichern als .env (gitignored).",
    "showSecretKeys": "Secret-Keys anzeigen",
    "secretBadge": "secret",
    "noEnvExample": "Keine .env.example oder .env.launchpad gefunden. Pro Service/Repo als Blueprint anlegen. Datastore-URLs können trotzdem gesetzt werden."
  },
  "envVars": {
    "title": "Variablen",
    "customTitle": "Eigene Umgebungsvariablen",
    "customBlurb": "Variablen hinzufügen, die dein Service zur Laufzeit braucht. Einen .env-Block einfügen oder einzeln hinzufügen. Wird über .env.example / .env.launchpad gelegt und in den Container injiziert.",
    "paste": ".env einfügen",
    "pasteHint": "KEY=VALUE-Zeilen einfügen (eine .env-Datei). Kommentare (#) und Leerzeilen werden ignoriert; vorhandene Keys werden aktualisiert.",
    "pasteParsed": "{count} Variable(n) erkannt",
    "pasteApply": "Erkannte hinzufügen",
    "add": "Variable hinzufügen",
    "remove": "Entfernen",
    "empty": "Noch keine eigenen Variablen.",
    "keyPlaceholder": "KEY",
    "valuePlaceholder": "wert",
    "reveal": "Wert anzeigen",
    "hide": "Wert verbergen",
    "corsNote": "Launchpad fügt die Preview-Frontend-URL beim Deploy automatisch zu den CORS-Origins des Backends hinzu (CORS_ALLOWED_ORIGINS) - angehängt, falls bereits gesetzt, sonst neu hinzugefügt - damit das Frontend das Backend ohne manuelle Einrichtung aufrufen kann."
  },
  "promotions": {
    "title": "Stage-Promotions",
    "blurb": "Umgebungen von Preview über Staging nach Production promoten. Production erfordert standardmäßig Freigabe.",
    "backToOrg": "Organisation",
    "policyTitle": "Freigabe-Richtlinie",
    "policyBlurb": "Steuern Sie, ob Staging- und Production-Promotions vor dem Deploy eine Org-Admin-Freigabe brauchen.",
    "stagingRequiresApproval": "Staging erfordert Freigabe",
    "productionRequiresApproval": "Production erfordert Freigabe",
    "savePolicy": "Richtlinie speichern",
    "policySaved": "Richtlinie gespeichert",
    "policySavedBlurb": "Freigaberegeln für diese Organisation aktualisiert.",
    "adminOnly": "Nur Owner und Admins können die Promotion-Richtlinie ändern.",
    "queueTitle": "Promotion-Warteschlange",
    "filterPending": "Ausstehend",
    "filterAll": "Alle",
    "empty": "Noch keine Promotion-Anfragen.",
    "openSource": "Quell-Umgebung öffnen",
    "openTarget": "Neue Umgebung öffnen",
    "approve": "Freigeben",
    "reject": "Ablehnen",
    "approved": "Promotion freigegeben",
    "approvedBlurb": "Die Zielumgebung wurde eingereiht.",
    "rejected": "Promotion abgelehnt",
    "rejectedBlurb": "Die Anfrage wurde ohne Deploy geschlossen."
  },
  "notifications": {
    "title": "Benachrichtigungen",
    "empty": "Alles erledigt.",
    "clearAll": "Alles leeren",
    "dismiss": "Schließen",
    "dismissAria": "Benachrichtigung schließen",
    "someone": "Ein Teammitglied",
    "inviteOrgTitle": "Organisationseinladung: {org}",
    "inviteOrgBody": "{by} hat dich als {role} eingeladen. Öffnen zum Annehmen.",
    "inviteProjectTitle": "Projekteinladung: {project}",
    "inviteProjectBody": "{by} hat dich zu {org} als {role} eingeladen. Öffnen zum Annehmen."
  },
  "environments": {
    "lifecycle": {
      "preview": "Preview",
      "staging": "Staging",
      "production": "Production"
    },
    "index": {
      "title": "Aktive Umgebungen",
      "estHourlySpend": "Geschätzte Stundenkosten",
      "orgCloudAccrued": "Org-Cloud bisher",
      "attention": "Aufmerksamkeit",
      "controlPlane": "Control Plane",
      "ephemeralTargets": "Ephemere K8s-Ziele",
      "live": "Live-Umgebungen",
      "lineageBlurb": "Gruppiert nach App: Preview, Staging, dann Production.",
      "lineageStages": "{count} Stufe | {count} Stufen",
      "launchPreview": "Preview starten",
      "loading": "Umgebungen werden geladen…",
      "empty": "Noch keine aktiven Umgebungen.",
      "firstPreview": "Erste Preview starten",
      "advancedForm": "Oder erweitertes Git-Formular nutzen",
      "systemLogs": "System-Logs",
      "totalCount": "{count} gesamt",
      "failedLabel": "fehlgeschlagen",
      "namespaceGroup": "Namespace-Gruppe",
      "namespaceGroups": "Namespace-Gruppen"
    },
    "preview": {
      "frontend": "Frontend",
      "backend": "API",
      "openTab": "In neuem Tab öffnen",
      "allUrls": "Preview-URLs",
      "alsoExposed": "Zusätzlich erreichbar"
    },
    "deployKind": {
      "title": "Provisionierungs-Laufzeit",
      "kubernetes": "Kubernetes",
      "docker": "Docker Compose",
      "instance": "Instanz"
    },
    "datastoreStatus": {
      "pending": "startet",
      "running": "verbunden",
      "failed": "fehlgeschlagen",
      "stopped": "gestoppt"
    },
    "detail": {
      "crumb": "Umgebungen",
      "openApp": "App öffnen",
      "provisioning": "App wird bereitgestellt…",
      "appUrl": "App-URL",
      "ttlRemaining": "TTL verbleibend",
      "noTtl": "Keine TTL",
      "ttlPending": "Startet, sobald aktiv",
      "noTtlHint": "Permanent bis Sie die Umgebung zerstören",
      "expires": "Läuft ab",
      "promoteToStaging": "Nach Staging promoten",
      "promoteToProduction": "Nach Production promoten",
      "stagePromoting": "Wird promoted…",
      "stagePromoteStarted": "Promotion nach {stage}",
      "stagePromoteStartedBlurb": "Die neue Umgebung wird geöffnet. Die Bereitstellung läuft im Hintergrund weiter.",
      "stagePromotePendingTitle": "Freigabe erforderlich",
      "stagePromotePendingApproval": "Promotion nach {stage} wartet auf einen Org-Admin. Prüfen Sie unter Stage-Promotions.",
      "stagePromoteFailed": "Promotion fehlgeschlagen",
      "confirmPromoteStagingTitle": "Nach Staging promoten?",
      "confirmPromoteStagingMessage": "Neue Staging-Umgebung aus „{name}“ erstellen. Die aktuelle Preview läuft weiter. Staging hat eine längere TTL (Standard 7 Tage) und nutzt dasselbe Repo, denselben Branch und Workspace.",
      "confirmPromoteStagingAction": "Ja, nach Staging promoten",
      "confirmPromoteProductionTitle": "Nach Production promoten?",
      "confirmPromoteProductionMessage": "Production-Umgebung aus „{name}“ erstellen. Production hat keine TTL und braucht meist eine Org-Admin-Freigabe vor dem Deploy. Die aktuelle Umgebung läuft weiter.",
      "confirmPromoteProductionAction": "Ja, nach Production promoten",
      "lifecycleStage": "Lebenszyklus-Stufe",
      "promotedFrom": "Promoted von",
      "viewPromotions": "Promotions anzeigen",
      "costToDate": "Kosten bisher",
      "localShadow": "Lokaler Schattenwert",
      "gitRepo": "Git-Repository",
      "gitBranch": "Git-Branch",
      "datastores": "Ephemere Datenspeicher",
      "stablePrUrl": "Stabile PR-URL",
      "linkedWorkspace": "Verknüpfter Workspace",
      "ttlWarning": "TTL 30 Minuten oder weniger - jetzt verlängern, um die Preview zu behalten.",
      "driftWarning": "Konfigurationsdrift zwischen Cluster und Launchpad erkannt.",
      "gitPushRebuilds": "Git-Push-Rebuilds",
      "deployToCloud": "In die Cloud deployen",
      "launchCloudPreview": "Cloud-Umgebung starten",
      "launchingCloud": "Cloud-Umgebung wird gestartet…",
      "softCostCapWarning": "Soft-Cost-Cap für diese Umgebung erreicht. Zerstören oder erhöhen",
      "openAppWhenRunning": "App-URL verfügbar, wenn Status Running ist",
      "ttlExpiredResumeDisabled": "TTL abgelaufen - Fortsetzen ist deaktiviert",
      "promoteBlurb": "Erstellt eine dauerhafte Cloud-Umgebung (ohne TTL) aus dieser Staging- oder Production-Quelle. Die lokale Umgebung läuft weiter, bis Sie sie zerstören.",
      "promotePrimaryService": "Preview-Service",
      "promotePrimaryServiceHint": "Bei Multi-Service-Apps wählen Sie, welcher Service die öffentliche Cloud-URL erhält. Wir empfehlen das Frontend, wenn erkannt.",
      "promoteRecommended": "empfohlen",
      "promoteServicesLoading": "Workspace-Services werden geladen…",
      "promoteCodeSource": "Code-Bereitstellung",
      "promoteCodeSourceHint": "Für pm2/systemd Instance-Modus: Workspace per SSH kopieren oder von GitHub auf der VM klonen. Docker baut und pusht weiterhin ein Image.",
      "promoteCodeSources": {
        "ssh": "SSH-Kopie",
        "github": "GitHub-Clone"
      },
      "promoteRegion": "Region",
      "promoteRegionHint": "Wo der GKE/EKS-Cluster, die Cloud-VM oder der Managed Service erstellt wird. Wählen Sie eine Region mit Kapazität nahe Ihren Nutzern.",
      "promoteNetworking": "Netzwerk",
      "promoteCreateVpc": "Neue VPC / VNet erstellen",
      "promoteCreateSubnets": "Subnets erstellen",
      "promoteUseExistingVpc": "Vorhandene VPC / Netzwerk nutzen",
      "promoteDefaultNetwork": "Standardnetzwerk des Providers",
      "promoteSelectVpc": "VPC / Netzwerk",
      "promoteSelectVpcPlaceholder": "VPC auswählen",
      "promoteNoVpcs": "Keine VPCs gefunden. Neue VPC erstellen oder Standardnetzwerk nutzen.",
      "promoteRefreshVpcs": "Aus der Cloud aktualisieren",
      "promoteNetworkingHint": "VPCs aus dem Cloud-Konto laden, eine neue isolierte VPC (optional mit Subnets) erstellen oder das Standardnetzwerk nutzen.",
      "promoteSecurityGroup": "Security Group (AWS)",
      "promoteCreateSecurityGroup": "Launchpad Security Group erstellen",
      "promoteUseExistingSecurityGroup": "Vorhandene Security Group nutzen",
      "promoteSelectSecurityGroup": "Security Group",
      "promoteSelectSecurityGroupPlaceholder": "Security Group auswählen",
      "promoteNoSecurityGroups": "Keine Security Groups in dieser Region gefunden.",
      "promoteRefreshSecurityGroups": "Aus der Cloud aktualisieren",
      "promoteSecurityGroupHint": "Launchpad stellt sicher, dass SSH (22) und der App-Port in der gewählten Gruppe erlaubt sind.",
      "promoteDeployTargets": "Cloud-Dienste für dieses Deploy",
      "promoteDeployTargetsHint": "Ressourcen, die Launchpad auf {provider} für diese Preview erstellt oder nutzt.",
      "promoteTargetCategories": {
        "compute": "Compute",
        "registry": "Registry",
        "network": "Netzwerk",
        "security": "Sicherheit",
        "platform": "Plattform"
      },
      "useStoredCredentials": "Gespeicherte Credentials verwenden",
      "useStoredCredentialsHint": "Secrets bleiben in den Einstellungen verschlüsselt. Felder leer lassen nutzt das Vault.",
      "appUrlWhenRunning": "Verfügbar bei Running - Logs unten beobachten",
      "postgres": "Postgres",
      "redis": "Redis",
      "datastoresInjected": "Eingespielt über app-secrets (DATABASE_URL / REDIS_URL)",
      "linkedPr": "Verknüpfter PR",
      "template": "Vorlage",
      "workloadImage": "Workload-Image",
      "environmentId": "Umgebungs-ID",
      "gitPushRebuildActive": "Push auf {branch} in diesem Repo rebuildet die Preview, solange die Umgebung aktiv ist.",
      "webhookConfigured": "Webhook ist konfiguriert -",
      "webhookSetup": "WEBHOOK_SECRET setzen und GitHub auf /api/v1/webhooks/github zeigen -",
      "latestCommit": "Letzter Commit: {sha}",
      "concurrentPreviews": "Gleichzeitige Previews: {active} / {max}",
      "needCustomManifests": "Eigene Manifeste oder Terraform nötig?",
      "openProvision": "Provision öffnen",
      "livePreviewOnly": "für einen IaC-Workspace - diese Seite ist nur für die Live-Preview.",
      "auditEmpty": "Noch keine Control-Plane-Audit-Ereignisse für diese Preview.",
      "openPreview": "Preview öffnen",
      "openPreviewTitle": "{image} öffnen ({port})",
      "deployingToCloud": "Deployment nach {provider}.",
      "regenerateDockerfile": {
        "button": "Dockerfile neu erzeugen und erneut versuchen",
        "hint": "Der Image-Build ist fehlgeschlagen. Launchpad kann ein zum Stack passendes Dockerfile erzeugen.",
        "title": "Dockerfile neu erzeugen und erneut versuchen?",
        "message": "Der Container-Image-Build ist fehlgeschlagen, oft weil das eigene Dockerfile des Repositorys auf Dateien verweist, die nicht im Build-Kontext liegen. Launchpad erzeugt ein neues Dockerfile aus dem erkannten Stack und baut erneut. Ihr Repository wird nicht verändert. Fortfahren?",
        "confirm": "Erzeugen und erneut versuchen",
        "toastTitle": "Neuer Build mit erzeugtem Dockerfile"
      }
    },
    "actions": {
      "pause": "Pausieren",
      "pausing": "Pausieren…",
      "resume": "Fortsetzen",
      "resuming": "Fortsetzen…",
      "expired": "Abgelaufen",
      "relaunch": "Erneut starten",
      "retry": "Provision erneut",
      "retrying": "Erneut versuchen…",
      "destroy": "Zerstören",
      "stopProvision": "Bereitstellung stoppen",
      "queuingTeardown": "Abbau wird geplant…",
      "queuingStop": "Stoppen…",
      "copyUrl": "URL kopieren",
      "copied": "Kopiert",
      "extendTtl": "TTL zurücksetzen",
      "extending": "Zurücksetzen…",
      "scanDrift": "Drift prüfen",
      "scanning": "Prüfen…",
      "analyze": "Analysieren",
      "analyzing": "Analysieren…"
    },
    "destroy": {
      "title": "Umgebung zerstören?",
      "titleStop": "Bereitstellung stoppen?",
      "message": "Preview \"{name}\" zerstören? Laufzeit-Ressourcen dieser Umgebung werden abgebaut. Bei Cloud-Previews über Scaffold-IaC wird Terraform/Pulumi destroy auf dem verknüpften Workspace ausgeführt, wenn keine anderen aktiven Umgebungen ihn teilen. Dies kann nicht rückgängig gemacht werden.",
      "messageProvisioning": "Bereitstellung von \"{name}\" stoppen? Laufende Arbeit wird abgebrochen. Bereits erstellte Ressourcen bleiben bestehen (spaeter Zerstoeren zum Abbauen).",
      "messageForce": "Preview \"{name}\" zerstören? Laufende Bereitstellung wird abgebrochen und erstellte Ressourcen werden abgebaut (einschliesslich Scaffold-IaC bei Cloud-Deploy). Dies kann nicht rückgängig gemacht werden.",
      "confirm": "Ja, zerstören",
      "confirmStop": "Ja, stoppen",
      "cancel": "Nein"
    },
    "card": {
      "openApp": "App öffnen",
      "pause": "Pausieren",
      "resume": "Fortsetzen",
      "destroy": "Zerstören",
      "logs": "Logs",
      "details": "Details",
      "remaining": "Verbleibend",
      "liveRebuild": "Live-Rebuild-Stream"
    },
    "create": {
      "name": "Name",
      "ttl": "TTL",
      "hours": "Stunden",
      "minutes": "Minuten",
      "gitUrl": "Git-Repository-URL",
      "launch": "Umgebung starten"
    },
    "health": {
      "healthy": "Gesund",
      "waking": "Wird geweckt",
      "starting": "Startet",
      "paused": "Pausiert",
      "unhealthy": "Ungesund",
      "stopped": "Gestoppt"
    },
    "status": {
      "provisioning": "BEREITSTELLUNG",
      "running": "LÄUFT",
      "paused": "PAUSIERT",
      "failed": "FEHLGESCHLAGEN",
      "destroyed": "ZERSTÖRT",
      "expired": "ABGELAUFEN",
      "teardown": "LÖSCHEN"
    },
    "toasts": {
      "paused": "Umgebung pausiert",
      "pauseFailed": "Pausieren fehlgeschlagen",
      "resumed": "Umgebung fortgesetzt",
      "resumeFailed": "Fortsetzen fehlgeschlagen",
      "relaunched": "Umgebung erneut gestartet",
      "relaunchFailed": "Erneutes Starten fehlgeschlagen",
      "destroyed": "Umgebung gelöscht",
      "destroyComplete": "Entfernung abgeschlossen. Zurück zur Übersicht.",
      "destroyFailed": "Zerstören fehlgeschlagen",
      "deletingBanner": "Umgebung wird gelöscht und Ressourcen freigegeben…",
      "stopped": "Bereitstellung gestoppt",
      "stopFailed": "Stoppen fehlgeschlagen",
      "extended": "TTL zurückgesetzt",
      "extendFailed": "TTL-Zurücksetzen fehlgeschlagen",
      "driftOk": "Kein Drift erkannt",
      "driftFailed": "Drift-Scan fehlgeschlagen",
      "cloudFailed": "Cloud-Preview fehlgeschlagen",
      "portRemap": "Port umgebogen"
    }
  },
  "launch": {
    "eyebrow": "One-Click-Preview",
    "title": "Preview-App starten",
    "blurb": "Ziel wählen, Repo verbinden und ephemere Umgebung starten. Für vollständige IaC-Workspaces Provision nutzen.",
    "useProvision": "Zu Provision →",
    "aiAnalyze": "KI analysieren & beheben",
    "chooseTarget": "Ziel wählen",
    "pickSource": "Quelle wählen",
    "launch": "Starten",
    "createOne": "Eine erstellen",
    "splash": {
      "preparingWorkspace": "Workspace-Preview wird vorbereitet",
      "fromWorkspace": "Workspace-Preview",
      "titleForWorkspace": "{name} starten",
      "blurbForWorkspace": "Ziel und TTL bestätigen, dann starten. Manifeste und Images kommen aus diesem Workspace.",
      "workspaceMissing": "Dieser Workspace wurde nicht gefunden. Anderen wählen oder ohne Workspace starten."
    },
    "repoUrl": "Repository-URL",
    "optionalLocal": "(optional für Lokal)",
    "branch": "Branch",
    "prNumber": "GitHub-PR-Nummer (optional)",
    "mrNumber": "GitLab-MR-Nummer (optional)",
    "prSyncBlurb": "Optionale PR/MR-Nummer aktiviert Smoke-Test-Sync für Preview-URLs.",
    "localTitle": "Lokal (Sandbox)",
    "localBlurb": "Auf dem Rechner - keine Cloud-Zugangsdaten nötig.",
    "targets": {
      "gcp": "Google Cloud",
      "aws": "Amazon Web Services",
      "azure": "Microsoft Azure",
      "cloudflare": "Cloudflare",
      "local": "Lokal (Sandbox)"
    },
    "hints": {
      "local": "Auf Ihrem Rechner - wir starten den lokalen Cluster bei Bedarf",
      "gcp": "SA-JSON oder keyless WIF",
      "aws": "Access Keys oder Role ARN",
      "azure": "Service Principal",
      "cloudflare": "API-Token"
    },
    "kind": {
      "checking": "Lokaler Cluster wird geprüft…",
      "unavailable": "Kind-Status nicht verfügbar."
    },
    "workspace": {
      "launchingFrom": "Start aus Workspace",
      "launchingFromDetail": "Hat der Workspace rohe Manifeste, wendet Launchpad sie in einen Preview-Namespace an (Manifest-Deploy). Sonst nutzt es das eingebaute Preview-Profil.",
      "launchingFromInstance": "Deploy-Modus Running Instance - Launchpad deployed den Workspace-Container auf das gewählte Compute-Ziel (kein Kubernetes-Apply).",
      "launchingFromCompose": "Deploy-Modus Docker Compose - Launchpad startet docker compose aus dem Workspace.",
      "launchingFromManifest": "Launchpad wendet Workspace-Kubernetes-Manifeste in einen Preview-Namespace an.",
      "linkBlurb": "Workspace verknüpfen, um Kubernetes-Manifeste zu deployen, oder unten eine Git-Repo-URL eingeben.",
      "launchingFromSkipGit": "Git im nächsten Schritt überspringen.",
      "storedCredentials": "falls verfügbar - Credential-Felder unten überspringen.",
      "usingStoredFrom": "Gespeicherte Zugangsdaten aus",
      "noneUseGitRepo": "Git-Repo nutzen",
      "planTitle": "Smart Preview-Plan",
      "planMode": "Deploy-Modus",
      "planRuntime": "Runtime",
      "planDeps": "Datastores aus dem Workspace"
    },
    "cloud": {
      "chooseTargetBlurb": "Lokal nutzt kind auf Ihrem Rechner. Cloud-Optionen verschlüsseln Zugangsdaten auf einen verknüpften Workspace oder nutzen einen bestehenden Provision-Workspace."
    },
    "pickSourceBlurb": "GitHub oder GitLab wählen, dann Repository und Branch angeben.",
    "prEnableBlurb": "Aktiviert stabile /pr/{n}-URL, Smoke- + Status-Checks, Sync-Rebuilds und Auto-Destroy beim Schließen.",
    "confirmBlurb": "Sie erhalten Status-Logs, TTL und einen Open-app-Link, wenn der Workload Running ist.",
    "containerImage": "Container-Image",
    "containerImageRequired": "Erforderlich, sofern nicht aus Workspace oder Repo-Build gestartet wird.",
    "containerImageFromWorkspace": "Image kommt aus dem Workspace (Dockerfile, Compose oder Manifeste). Kein manuelles Image nötig.",
    "kubernetesImageSource": "Container-Image-Quelle",
    "kubernetesBuildRegistry": "Aus Workspace bauen und in Registry pushen",
    "kubernetesBuildRegistryLocalHint": "Baut Dockerfiles aus dem Workspace und lädt Images in den lokalen kind/k3d-Cluster.",
    "kubernetesBuildRegistryCloudHint": "Stellt {provider} Container Registry bereit (ECR / Artifact Registry), baut Workspace-Images, pusht sie und patcht Kubernetes-Manifeste.",
    "kubernetesExternalImage": "Externes Container-Image verwenden",
    "kubernetesExternalImageHint": "Kein Build. Deploy mit einer fertigen Image-Referenz (z. B. ghcr.io/org/app:tag).",
    "imageScan": {
      "title": "Image vor dem Deploy auf CVEs scannen",
      "blurb": "Führt Trivy gegen das gebaute Image aus und kann das Cloud-Deploy bei CRITICAL- oder HIGH-Funden blockieren."
    },
    "afterLaunchRebuild": "Nach dem Start: Push auf {branch} rebuildet die Preview, wenn ein GitHub-Webhook konfiguriert ist.",
    "yourBranch": "Ihr Branch",
    "ephemeralDatastores": "Ephemere Datenspeicher (opt-in)",
    "postgresInCluster": "Postgres (im Cluster)",
    "redisInCluster": "Redis (im Cluster)",
    "summary": {
      "estHourly": "Gesch. stündlich",
      "image": "Image",
      "rebuilds": "Rebuilds",
      "rebuildsBlurb": "Push auf Branch, wenn Webhook konfiguriert ist",
      "workspace": "Workspace: {name}",
      "imageRef": "Image {image}",
      "noImage": "Kein Image (Container-Image setzen oder Workspace/Repo nutzen)",
      "builtFrom": "Gebaut aus {dockerfile}",
      "fromManifests": "Aus Workspace-Manifesten",
      "fromWorkspace": "Aus Workspace",
      "fromWorkspaceInstance": "Aus Workspace (laufende Instanz)",
      "fromWorkspaceCompose": "Aus Workspace (Docker Compose)",
      "required": "Erforderlich"
    },
    "errors": {
      "launchFailed": "Start fehlgeschlagen",
      "loadFailed": "Launch-Optionen konnten nicht geladen werden",
      "kindCheckFailed": "Lokaler Cluster konnte nicht geprüft werden",
      "pasteCredentials": "Cloud-Zugangsdaten einfügen, um fortzufahren",
      "urlMustMatchGithub": "URL muss zum gewählten GitHub-Anbieter passen",
      "urlMustMatchGitlab": "URL muss zum gewählten GitLab-Anbieter passen",
      "invalidRepo": "Gültige Git-Repo-URL und Branch eingeben",
      "invalidName": "Name muss kleingeschrieben sein, mit Buchstabe beginnen, 3-64 Zeichen",
      "invalidWorkspaceSource": "Workspace-Quelle ist ungültig",
      "setContainerImage": "Container-Image setzen (oder Git-Repo-URL und Branch)",
      "localClusterNotReady": "Lokaler Sandbox-Cluster ist nicht bereit. Tools/Cluster vor dem Start reparieren.",
      "containerImageRequired": "Container-Image ist erforderlich, wenn nicht aus Workspace oder Repo-Build gestartet wird",
      "containerImageRequiredLocal": "Container-Image ist für reine Image-Starts erforderlich"
    }
  },
  "invite": {
    "title": "Organisationseinladung",
    "blurb": "Du wurdest eingeladen, einer Organisation auf Launchpad beizutreten.",
    "working": "Arbeitet…",
    "failed": "Einladung konnte nicht angenommen werden",
    "accepted": "Einladung angenommen",
    "joinedAs": "{org} als {role} beigetreten.",
    "needAccount": "Erstelle ein Konto oder melde dich mit der eingeladenen E-Mail an, dann annehmen.",
    "registerToAccept": "Konto erstellen",
    "loginToAccept": "Anmelden",
    "acceptOrg": "Einladung annehmen",
    "continueOrg": "Zur Organisation",
    "projectTitle": "Projekteinladung",
    "projectBlurb": "Du wurdest eingeladen, an einem Projekt mitzuarbeiten.",
    "projectJoined": "{project} als {role} beigetreten.",
    "acceptProject": "Projekteinladung annehmen",
    "continueProject": "Zum Projekt"
  },
  "onboarding": {
    "orgTitle": "Organisation erstellen",
    "orgBlurb": "Organisationen besitzen Projekte, Workspaces und Abrechnung. Benenne deine Organisation, um zu starten.",
    "orgName": "Organisationsname",
    "orgContinue": "Organisation erstellen",
    "orgFailed": "Organisation konnte nicht erstellt werden"
  },
  "projects": {
    "eyebrow": "Projekte",
    "title": "Projekte",
    "blurb": "Workspaces unter Projekten gruppieren. Owner und Admins können Projekte anlegen und einladen.",
    "planUsage": "Plan {plan}: {count} / {max} Projekte",
    "createTitle": "Projekt erstellen",
    "name": "Projektname",
    "create": "Projekt erstellen",
    "loading": "Projekte werden geladen…",
    "empty": "Noch keine Projekte.",
    "workspaceCount": "{count} Workspaces",
    "back": "Projekte",
    "workspacesTitle": "Workspaces",
    "workspacesBlurb": "Sandboxes und importierte Repos in diesem Projekt.",
    "viewWorkspaces": "Workspaces öffnen",
    "createWorkspace": "Neuer Workspace",
    "importRepo": "Repo importieren",
    "emptyWorkspaces": "Noch keine Workspaces in diesem Projekt.",
    "openWorkspace": "Öffnen",
    "members": "Mitglieder",
    "invites": "Einladungen",
    "sendInvite": "Einladung senden",
    "rename": "Umbenennen",
    "saveName": "Namen speichern",
    "renamed": "Projekt umbenannt",
    "selectOrgMember": "Teammitglied wählen…",
    "inviteOrgMembersBlurb": "Lade Personen ein, die bereits zur Organisation gehören.",
    "noInviteCandidates": "Alle Organisationsmitglieder sind bereits in diesem Projekt.",
    "inviteEmailed": "Einladung per E-Mail an {email} gesendet",
    "inviteEmailFailed": "Einladung für {email} erstellt, E-Mail fehlgeschlagen ({error}). Link: {url}",
    "inviteCreatedSmtp": "Einladung erstellt. Link teilen: {url}",
    "inviteCreatedFor": "Einladung erstellt für {email}",
    "roleUpdated": "{email} auf {role} aktualisiert",
    "errors": {
      "load": "Projekte konnten nicht geladen werden",
      "create": "Projekt konnte nicht erstellt werden",
      "invite": "Einladung fehlgeschlagen",
      "revoke": "Widerruf fehlgeschlagen",
      "rename": "Umbenennen fehlgeschlagen",
      "roleUpdate": "Rollenaktualisierung fehlgeschlagen"
    }
  },
  "provision": {
    "eyebrow": "Infrastruktur",
    "title": "Cloud-Umgebung provisionieren",
    "blurb": "Deploy-Target waehlen, standardmaessig mit LaunchProvision provisionieren oder zu Terraform / Pulumi wechseln. Zugangsdaten kommen aus den Einstellungen, sofern Sie sie nicht pro Workspace ueberschreiben.",
    "workspace": "Workspace",
    "createNew": "+ Neuen Workspace erstellen",
    "workspaceName": "Workspace-Name",
    "launchpadProject": "Projekt",
    "selectProject": "Projekt wählen…",
    "launchpadProjectBlurb": "Neue Workspaces gehören zu einem Launchpad-Projekt.",
    "manageProjects": "Projekte verwalten",
    "iacEngine": "Provision-Engine",
    "iacEngines": {
      "launchpad": "LaunchProvision (Standard)"
    },
    "iacEngineLaunchpadHint": "Stellt nur bereit, was dieser Dienst braucht (VPC, Cluster, Registry, Secrets). Im Workspace-Terminal: bash infra/launchProvision.sh up.",
    "continue": "Weiter",
    "generate": "Workspace erzeugen",
    "saveOpenTerminal": "Speichern & Terminal öffnen",
    "providers": {
      "local": "Lokal",
      "gcp": "Google Cloud",
      "aws": "Amazon Web Services",
      "azure": "Microsoft Azure",
      "cloudflare": "Cloudflare"
    },
    "topology": {
      "simple": "Einfach - einzelne VPC",
      "standard": "Standard - Hub und Spoke"
    },
    "secrets": {
      "backend": "Secret-Backend",
      "manager": "Secret Manager",
      "k8s": "Natives K8s-Secret"
    },
    "sandbox": {
      "title": "Lokales Sandbox-Setup prüfen",
      "blurb": "Auf dem Rechner - keine Cloud-Zugangsdaten nötig. Ein lokaler Cluster startet nur bei Kubernetes."
    },
    "runtimeMode": {
      "title": "Laufzeit",
      "blurb": "Wähle, wie Previews laufen. Eine Laufzeit gewinnt; Packaging folgt ihr.",
      "modes": {
        "kubernetes": {
          "title": "Kubernetes",
          "desc": "Lokaler Cluster oder GKE/EKS/AKS in der Cloud"
        },
        "docker_compose": {
          "title": "Docker Compose",
          "desc": "Nur lokales Docker - kein entfernter Socket"
        },
        "running_instance": {
          "title": "Laufende Instanz",
          "desc": "Cloud Run, EC2/VPS oder lokale Maschine"
        }
      },
      "composeHint": {
        "title": "Compose-Preview-Pfad",
        "blurb": "Dockerfile, docker-compose.yml, optionales IaC (Terraform/Pulumi) und CI/CD (GitHub/GitLab) können erzeugt werden. Preview überspringt den lokalen Kubernetes-Cluster."
      },
      "attach": {
        "title": "Compute-Ziel",
        "blurb": "Wähle den Cloud-Dienst, der die App ausführt. Die Preview-URL entsteht nach dem Deploy.",
        "dockerNote": "Serverless nutzt immer Docker/OCI. Auf VMs und lokal: Docker, systemd oder PM2, optional nginx/Caddy davor.",
        "cloudflareNote": "Cloudflare Workers/Pages sind Edge-Runtimes, keine Docker-Hosts. VM per SSH nutzen oder Kubernetes / Workers in den IaC-Diensten.",
        "containerBadge": "Container-Image",
        "processStrategy": "Prozessstrategie",
        "reverseProxy": "Reverse Proxy",
        "reverseProxyHint": "TLS und Routing auf den App-Port. Die App selbst nicht als nginx betreiben.",
        "strategies": {
          "docker": "Docker (empfohlen)",
          "systemd": "systemd-Unit",
          "pm2": "PM2 (Node)"
        },
        "strategyHints": {
          "docker": "One-Click Launch Preview. Gleiches Image wie Compose/K8s.",
          "systemd": "Beste native Überwachung auf Linux-VMs. Scaffolds unter infra/instance/.",
          "pm2": "Node-Prozessmanager. Schreibt ecosystem.config.cjs."
        },
        "codeSource": "Code-Bereitstellung",
        "codeSources": {
          "ssh": "SSH-Kopie (Workspace-Dateien)",
          "github": "GitHub-Clone"
        },
        "codeSourceHints": {
          "ssh": "Workspace per SSH (rsync/scp) auf die VM kopieren.",
          "github": "Git-Repository der Umgebung auf der VM klonen."
        },
        "proxies": {
          "none": "Keine",
          "nginx": "nginx",
          "caddy": "Caddy"
        },
        "localByStrategy": {
          "docker": {
            "title": "Lokales Docker",
            "desc": "docker run auf dieser Maschine",
            "badge": "Container-Image"
          },
          "systemd": {
            "title": "Lokales systemd",
            "desc": "Nativer Prozess per systemd-Unit auf dieser Maschine",
            "badge": "systemd-Unit"
          },
          "pm2": {
            "title": "Lokales PM2",
            "desc": "Node-Prozessmanager (ecosystem.config.cjs) auf dieser Maschine",
            "badge": "PM2-Prozess"
          }
        },
        "vmByStrategy": {
          "docker": {
            "title": "Remote-VM (SSH)",
            "desc": "Jeder Linux-Host mit Docker",
            "badge": "Container-Image"
          },
          "systemd": {
            "title": "Remote-VM - systemd",
            "desc": "SSH-Host mit systemd-Unit (Ansible / infra/instance)",
            "badge": "systemd-Unit"
          },
          "pm2": {
            "title": "Remote-VM - PM2",
            "desc": "SSH-Host mit PM2 (ecosystem.config.cjs)",
            "badge": "PM2-Prozess"
          }
        },
        "serverlessHint": "Deployt das Container-Image zum Managed Service (gcloud / AWS wenn verfügbar).",
        "serverlessConfigHint": "Benötigt Container-Image, Cloud-Zugangsdaten und Region. Artifact Registry / ECR empfohlen.",
        "vmConfigHint": "SSH von der Control Plane nötig. Docker-Strategie braucht Docker auf der VM; systemd/PM2 nutzen infra/instance.",
        "localMachineHint": "Docker-Strategie läuft lokal per docker. systemd/PM2-Scaffolds unter infra/instance/.",
        "localBlurb": "Lokales Docker oder Remote-VM per SSH wählen. Optionale IaC- und CI/CD-Scaffolds finden Sie darunter.",
        "kind": "Compute-Art",
        "serviceName": "Service-Name",
        "region": "Region",
        "host": "Host (IP oder Hostname)",
        "sshUser": "SSH-Benutzer",
        "sshPort": "SSH-Port",
        "sshKeyPath": "Pfad zum SSH-Privtschlüssel (Control Plane)",
        "listenPort": "Host-Publish-Port",
        "endpointUrl": "Preview-URL-Override (optional)",
        "kinds": {
          "serverless": "Cloud Run / Container Apps",
          "vm": "VM / VPS / EC2 (SSH)",
          "local_machine": "Lokale Maschine (Docker)"
        },
        "targets": {
          "local_docker": {
            "title": "Lokales Docker",
            "desc": "docker run auf dieser Maschine"
          },
          "local_vm_ssh": {
            "title": "Remote-VM (SSH)",
            "desc": "Beliebiger Linux-Host mit Docker"
          },
          "gcp_cloud_run": {
            "title": "Cloud Run",
            "desc": "Serverless-Container aus Docker-Image"
          },
          "gcp_vm_ssh": {
            "title": "Compute Engine / VM (SSH)",
            "desc": "SSH + Docker auf GCE oder anderer VM"
          },
          "aws_app_runner": {
            "title": "App Runner",
            "desc": "Managed Container aus ECR / öffentlichen Images"
          },
          "aws_ec2": {
            "title": "EC2 (SSH + Docker)",
            "desc": "Volle VM-Kontrolle; Docker auf der Instanz"
          },
          "azure_container_apps": {
            "title": "Container Apps",
            "desc": "Serverless-Container auf Azure"
          },
          "azure_vm_ssh": {
            "title": "Azure-VM (SSH)",
            "desc": "SSH + Docker auf einer Linux-VM"
          },
          "cloudflare_vm_ssh": {
            "title": "Eigene VM (SSH)",
            "desc": "Workers sind kein Docker; SSH-Docker woanders nutzen"
          }
        }
      }
    },
    "cta": {
      "startLocal": "Lokalen Cluster & Sandbox starten",
      "generateLaunch": "Erzeugen & Sandbox starten",
      "busy": "Wird ausgeführt…"
    },
    "steps": {
      "validate": "Cloud-Konfiguration prüfen",
      "generate": "IaC & Container-Scaffolds erzeugen",
      "sandbox": "Sandbox-Sitzung vorbereiten"
    },
    "stepTitles": {
      "step1": "Workspace & Cloud wählen",
      "step2": "{provider}-Ressourcen & Zugangsdaten",
      "step3New": "Erzeugen & Sandbox starten",
      "step3Existing": "Änderungen speichern & Sandbox öffnen"
    },
    "stepProgress": "SCHRITT {current}/{total}",
    "workspaceSelectBlurb": "Neuen Stack erstellen oder bestehenden mit vorherigen Einstellungen wieder öffnen.",
    "loadingWorkspaceConfig": "Workspace-Einstellungen werden geladen…",
    "promote": {
      "enable": "Diesen Workspace als neue Umgebung promoten",
      "target": "Promotion-Ziel",
      "staging": "Staging",
      "prod": "Produktion",
      "name": "Name des promoteten Workspace"
    },
    "cost": {
      "label": "Live-Kostenschätzung vor dem Erzeugen",
      "hourly": "Geschätzt pro Stunde",
      "monthly": "Geschätzt pro Monat",
      "breakdown": "Geschätzte Aufschlüsselung",
      "refreshing": "Schätzung wird aktualisiert…",
      "estimateFailed": "Schätzung konnte gerade nicht aktualisiert werden."
    },
    "iacEngineLocalHint": "Lokales Kubernetes scaffoldet nur Manifeste - zu Cloud-Anbieter wechseln fuer LaunchProvision, Terraform, OpenTofu oder Pulumi.",
    "iacEngineLocalRuntimeHint": "Lokale Compose- und Running-Instance-Workspaces scaffolden Terraform-, OpenTofu- oder Pulumi-Stubs plus optionales CI/CD. Fuer LaunchProvision oder Managed Modules zu einem Cloud-Anbieter wechseln.",
    "editingExisting": "Bearbeiten",
    "editingExistingSuffix": "vorherige Ressourcenauswahl wird im nächsten Schritt geladen. Zugangsdaten leer lassen, um gespeicherte zu behalten.",
    "credentialsStored": "Kontoschlüssel aus den Einstellungen wurden für diesen Provider erkannt. Anmeldefelder leer lassen, um sie zu nutzen, oder neue Werte einfügen, um sie für diesen Workspace zu überschreiben.",
    "credentialsDetectedTitle": "Gespeicherte Cloud-Zugangsdaten erkannt",
    "infraGeneration": "Infrastruktur-Generierung",
    "providerBlurbs": {
      "aws": "Enterprise-Skalierung mit EKS, EC2 und Secrets Manager.",
      "gcp": "Erstklassiges GKE, Artifact Registry und Cloud Run.",
      "azure": "AKS, Key Vault und Container Apps für MS-Stacks.",
      "cloudflare": "Niedrige Latenz mit Workers, R2 und Edge-DNS."
    },
    "fields": {
      "projectId": "Projekt-ID",
      "region": "Region",
      "networkTopology": "Netzwerktopologie",
      "instanceMachineType": "Instanz- / Maschinentyp",
      "instanceType": "Instanztyp",
      "resourceGroup": "Ressourcengruppe",
      "location": "Standort",
      "vmSize": "VM-Größe",
      "accountId": "Account-ID",
      "zoneNameOptional": "Zonenname (optional)",
      "clusterName": "Cluster-Name",
      "kubectlContext": "kubectl-Kontext",
      "services": "Cloud-Services"
    },
    "resources": {
      "gcp": {
        "vpc": { "title": "VPC", "desc": "Isoliertes Netzwerk mit Custom Routing" },
        "subnets": { "title": "Subnets", "desc": "Regionales Subnet-Layout" },
        "gke": { "title": "GKE", "desc": "Managed Kubernetes-Cluster" },
        "artifact_registry": { "title": "Artifact Registry", "desc": "Container-Image-Speicher" },
        "cloud_run": { "title": "Cloud Run", "desc": "Serverlose Container" },
        "cloud_functions": { "title": "Cloud Functions", "desc": "Event-gesteuerte Funktionen" },
        "cloud_sql": { "title": "Cloud SQL", "desc": "Managed PostgreSQL / MySQL" },
        "cloud_storage": { "title": "Cloud Storage", "desc": "Object-Storage-Buckets" },
        "pubsub": { "title": "Pub/Sub", "desc": "Messaging-Topics & Subscriptions" },
        "memorystore": { "title": "Memorystore", "desc": "Managed Redis" },
        "bigquery": { "title": "BigQuery", "desc": "Analytics-Warehouse-Dataset" }
      },
      "aws": {
        "vpc": { "title": "VPC" },
        "subnets": { "title": "Subnets" },
        "ec2": { "title": "EC2" },
        "s3": { "title": "S3" },
        "eks": { "title": "EKS" },
        "secrets_manager": { "title": "Secrets Manager" },
        "rds": { "title": "RDS" },
        "ecr": { "title": "ECR" },
        "app_runner": {
          "title": "App Runner",
          "desc": "Managed Container aus Docker-Image"
        },
        "elasticache": { "title": "ElastiCache" },
        "lambda_fn": { "title": "Lambda" },
        "dynamodb": { "title": "DynamoDB" },
        "sqs": { "title": "SQS" },
        "alb": { "title": "ALB" }
      },
      "azure": {
        "vnet": { "title": "VNet" },
        "subnets": { "title": "Subnets" },
        "aks": { "title": "AKS" },
        "key_vault": { "title": "Key Vault" },
        "container_apps": { "title": "Container Apps" },
        "acr": { "title": "ACR" },
        "storage_account": { "title": "Storage Account" },
        "cosmos_db": { "title": "Cosmos DB" },
        "redis_cache": { "title": "Redis Cache" },
        "app_service": { "title": "App Service" },
        "log_analytics": { "title": "Log Analytics" }
      },
      "cloudflare": {
        "workers": { "title": "Workers" },
        "r2": { "title": "R2" },
        "dns_records": { "title": "DNS" },
        "pages": { "title": "Pages" },
        "kv": { "title": "KV" },
        "d1": { "title": "D1" },
        "tunnels": { "title": "Tunnel" },
        "queues": { "title": "Queues" }
      }
    },
    "sandboxSteps": {
      "kindUp": "Launchpad führt make kind-up beim Provisionieren automatisch aus (benötigt Docker, kind und kubectl).",
      "envVars": "KUBERNETES_ENABLED=true und KUBERNETES_CONTEXT={context} in apps/api/.env behalten, dann API + Worker einmal neu starten.",
      "applyDestroy": "Sandbox öffnen - kubectl apply läuft gegen kind. Zerstören des letzten Dev-Workspaces führt make kind-down aus.",
      "switchProvider": "Wenn alles passt, Anbieter auf GCP/AWS/Azure/Cloudflare wechseln und speichern."
    },
    "github": {
      "privateRepo": "Privates Repository",
      "setCloudSecrets": "Cloud-CI-Secrets setzen",
      "addWorkflow": "Deploy-Workflow hinzufügen",
      "addDockerfiles": "Docker-Dateien hinzufügen",
      "repoBlurb": "Jetzt Repository wählen oder erstellen. Im letzten Schritt pusht Launchpad erzeugte Infra und fügt optional eine neue Workflow-Datei hinzu, ohne bestehende zu überschreiben.",
      "savingInfra": "Infra wird nach GitHub gespeichert…",
      "savingInfraWorkflow": "Infra und Workflow werden nach GitHub gespeichert…",
      "workflowSkipped": "Workflow übersprungen",
      "repoCreated": "Repository erstellt und Infra gepusht",
      "infraPushed": "Infra gepusht nach {repo}",
      "provisioningFailed": "GitHub-Provisionierung fehlgeschlagen",
      "loadStatusFailed": "GitHub-App-Status konnte nicht geladen werden"
    },
    "review": {
      "title": "Prüfen",
      "provider": "Anbieter",
      "engine": "Engine",
      "artifacts": "Artefakte",
      "provision": "Provision",
      "kubernetes": "Kubernetes",
      "cicd": "CI/CD",
      "githubRepo": "GitHub-Repo",
      "deployWorkflow": "Deploy-Workflow",
      "dockerFiles": "Docker-Dateien",
      "existing": "(bestehend)",
      "import": "(import)",
      "create": "(erstellen)",
      "skip": "(überspringen)",
      "skipped": "übersprungen"
    },
    "runInit": {
      "localTitle": "kubectl apply in Sandbox beim Öffnen ausführen",
      "composeTitle": "docker compose up in Sandbox beim Öffnen ausführen",
      "instanceTitle": "Beim Öffnen auf Compute-Ziel deployen",
      "cloudTitle": "IaC-Init in Sandbox beim Öffnen ausführen",
      "localBlurb": "Wendet Kubernetes-Manifeste (und Helm-Chart falls gewählt) gegen Ihren lokalen Cluster-Kontext an.",
      "composeBlurb": "Startet Dienste aus docker-compose.yml im Sandbox-Terminal.",
      "instanceBlurb": "Deployt das Container-Image auf das gewählte Compute-Ziel (lokales Docker, VM oder Managed Service).",
      "cloudBlurb": "Führt terraform/pulumi init in der sandboxed Terminal-Sitzung aus."
    },
    "progress": {
      "workspaceReady": "Workspace bereit",
      "configuring": "Dateien, Container-Scaffolds und Sandbox werden konfiguriert",
      "openingWorkspace": "Workspace-Details werden geöffnet ({id})…",
      "ready": "Bereit 🚀",
      "openWorkspace": "Workspace-Details öffnen",
      "workspaceId": "workspace {id}",
      "filesSummary": "{engine} / {provider} · {count} Dateien"
    },
    "credentialsHints": {
      "leaveBlank": "Leer lassen, um gespeicherte Zugangsdaten zu behalten",
      "pasteSaJson": "Service-Account-JSON einfügen (verschlüsselt gespeichert)"
    },
    "errors": {
      "failed": "Provisionierung fehlgeschlagen",
      "loadFailed": "Workspace-Konfiguration konnte nicht geladen werden",
      "selectWorkspace": "Workspace auswählen.",
      "selectProject": "Projekt für diesen Workspace wählen.",
      "workspaceNameMin": "Workspace-Name muss mindestens 3 Zeichen haben (kleingeschrieben, Bindestriche).",
      "workspaceNameFormat": "Name muss mit Buchstabe beginnen und nur a-z, 0-9, Bindestriche nutzen.",
      "localClusterRequired": "Lokaler Cluster-Name und kubectl-Kontext sind erforderlich.",
      "enableK8sGeneration": "Kubernetes-Generierung für lokale Kubernetes-Workspaces aktivieren.",
      "runtimeKubeContext": "local_machine läuft per Docker auf diesem Host (kein kube-Kontext).",
      "runtimeEndpoint": "Preview-URL-Override ist optional; nur zum Verknüpfen einer bestehenden URL nutzen.",
      "runtimeServerless": "Cloud Run, App Runner oder Container Apps für Serverless-Compute aktivieren.",
      "runtimeVmHost": "VM-Host (IP oder Hostname) ist erforderlich.",
      "runtimeLocalMachine": "local_machine ist nur für den lokalen Provider verfügbar.",
      "enableProvisionOrK8s": "Mindestens Provision oder Kubernetes-Generierung aktivieren.",
      "gcpProjectRequired": "GCP-Projekt-ID ist erforderlich.",
      "azureResourceGroupRequired": "Azure-Ressourcengruppe ist erforderlich.",
      "cloudflareAccountRequired": "Cloudflare-Account-ID ist erforderlich.",
      "promotedNameInvalid": "Name des promoteten Workspace darf nur Kleinbuchstaben, Zahlen und Bindestriche enthalten.",
      "invalidForm": "Ungültiges Formular",
      "invalidGithubForm": "Ungültiges GitHub-Formular",
      "timeoutCreated": "Workspace wurde erstellt, aber ein späterer Schritt ist abgelaufen. Erneut Erzeugen klicken zum Abschließen - kein Duplikat wird erstellt.",
      "timeoutContinue": "Ein Schritt ist nach Workspace-Erstellung abgelaufen. Erneut Erzeugen klicken zum Fortfahren - kein Duplikat wird erstellt."
    }
  },
  "workspaces": {
    "index": {
      "title": "Workspaces",
      "blurb": "Importierte Repos und erzeugte IaC-Sandboxes.",
      "import": "Repo importieren",
      "create": "Neuer Workspace",
      "empty": "Noch keine Workspaces.",
      "emptyForProject": "Noch keine Workspaces in diesem Projekt.",
      "importCta": "Repo importieren",
      "createCta": "Einen erstellen",
      "open": "Öffnen",
      "destroy": "Zerstören",
      "filteredByProject": "Gefiltert nach Projekt",
      "backToProject": "Zurück zum Projekt",
      "clearProjectFilter": "Alle Workspaces zeigen",
      "deleting": "Wird gelöscht…",
      "destroyFailed": "Löschen fehlgeschlagen",
      "retryDestroy": "Löschen erneut versuchen"
    },
    "destroy": {
      "title": "Workspace zerstören?",
      "message": "Workspace „{name}“ zerstören? Verknüpfte Environments und von Launchpad verwaltete Cloud-Ressourcen werden abgebaut. Generierte Dateien und die Sandbox werden entfernt. Dies kann nicht rückgängig gemacht werden.",
      "confirm": "Ja, zerstören",
      "cancel": "Nein"
    },
    "detail": {
      "loading": "Workspace wird geladen…",
      "loadingSetup": "Setup-Formular wird geladen…",
      "loadingK8s": "Kubernetes Visual Suite wird initialisiert…",
      "loadingIde": "Workspace-IDE wird geladen…",
      "kubernetesSuite": "Kubernetes Suite",
      "composeSuite": "Compose Suite",
      "instanceSuite": "Instanz Suite",
      "manifestConfigurator": "Manifest-Konfigurator"
    },
    "update": {
      "action": "Workspace aktualisieren",
      "eyebrow": "Workspace",
      "title": "Workspace aktualisieren",
      "blurb": "Cloud-Services, Runtime-Modus und Scaffold-Einstellungen für diesen Workspace ändern.",
      "blurbNamed": "Cloud-Services, Runtime-Modus und Scaffold-Einstellungen für {name} ändern.",
      "back": "Zurück zum Workspace",
      "saved": "Workspace-Konfiguration aktualisiert"
    },
    "linkedRepo": {
      "title": "Verknüpftes App-Repository",
      "blurb": "Verbinde das GitHub-Repo mit deinem App-Code. Launchpad verfolgt den gewählten Branch und aktualisiert Cloud-Umgebungen bei Push.",
      "branch": "Branch verfolgen",
      "cdMode": "Continuous Deploy",
      "modeWebhook": "Webhook (Standard)",
      "modeWebhookBlurb": "GitHub-Push-Webhook lässt Launchpad passende Umgebungen neu bauen. Keine Workflow-Änderungen im App-Repo.",
      "modeActions": "GitHub Actions",
      "modeActionsBlurb": "Launchpad schreibt einen Workflow ins App-Repo, der Launchpad bei Push auf den Branch benachrichtigt.",
      "save": "Verknüpfung speichern",
      "unlink": "Trennen",
      "saved": "App-Repo verknüpft",
      "unlinked": "App-Repo getrennt",
      "loadFailed": "Verknüpftes App-Repo konnte nicht geladen werden",
      "saveFailed": "Verknüpftes App-Repo konnte nicht gespeichert werden",
      "repoRequired": "GitHub-Installation und Repository wählen",
      "connectGithub": "Zuerst GitHub verbinden:",
      "webhookOk": "WEBHOOK_SECRET gesetzt",
      "webhookMissing": "WEBHOOK_SECRET fehlt",
      "workflow": "Workflow"
    },
    "errors": {
      "load": "Workspace konnte nicht geladen werden",
      "destroy": "Workspace konnte nicht zerstört werden",
      "terminal": "Terminal konnte nicht geöffnet werden",
      "star": "Workspace-Markierung konnte nicht aktualisiert werden"
    }
  },
  "catalog": {
    "index": {
      "eyebrow": "Golden Paths",
      "title": "Katalog",
      "blurb": "Freigegebene Vorlagen und Workspaces, die du im Katalog markierst.",
      "create": "Workspace erstellen",
      "approved": "Freigegebene Vorlagen",
      "yours": "Markierte Workspaces",
      "useTemplate": "Workspace erstellen",
      "empty": "Noch keine markierten Workspaces. Markiere einen Workspace unter Workspaces oder erstelle einen aus einer Golden-Path-Vorlage.",
      "unstar": "{name} demarkieren",
      "unstarAction": "Demarkieren",
      "loading": "Katalog wird geladen…",
      "tabsAria": "Katalogbereiche",
      "stacks": "Stacks",
      "dockerImages": "Docker-Images",
      "score": "Score {score}",
      "deleteConfirm": "\"{name}\" aus Markierte Workspaces entfernen? Entfernt nur den Katalog-Favoriten. Der verknüpfte Workspace bleibt erhalten, sofern nicht separat zerstört."
    },
    "create": {
      "title": "Workspace erstellen",
      "template": "Golden-Path-Vorlage",
      "name": "Workspace-Name",
      "owner": "Owner",
      "vcs": "Versionskontrolle",
      "back": "← Katalog",
      "blurb": "Erzeugt einen Workspace (Dockerfile, Kubernetes-Manifeste, CI-Sicherheitsscans) aus einer freigegebenen Golden-Path-Vorlage und markiert ihn in deinem Katalog.",
      "description": "Beschreibung",
      "tier": "Tier",
      "sloTarget": "SLO-Ziel %",
      "runbookUrl": "Runbook-URL",
      "onCall": "On-Call",
      "nameHint": "Wird auch als Repository-Name beim Erstellen oder Pushen eines Repos verwendet.",
      "vcsNone": "Keine (nur lokales IaC)",
      "createGithubRepo": "GitHub-Repository erstellen / pushen",
      "githubInstallation": "GitHub-Installation",
      "personalGithubWarning": "Persönliche GitHub-Konten können über App-Installationstokens keine neuen Repos erstellen. Zuerst leeres Repo {name} anlegen ({openGithub}), App-Zugriff gewähren, dann Erstellen erneut versuchen. Org-Installationen können Repos direkt erstellen.",
      "openGithub": "GitHub öffnen",
      "privateRepo": "Privates Repository",
      "gitlabNotConnected": "GitLab ist nicht verbunden.",
      "gitlabConnected": "Verbunden als {username} ({baseUrl})",
      "gitlabReturn": "unter Integrationen, dann hierher zurück.",
      "gitlabProjectName": "GitLab-Projektname",
      "privateProject": "Privates Projekt",
      "gitlabCiBlurb": "Erstellt das Projekt und committed eine Golden-Path-.gitlab-ci.yml (Semgrep + Build + Trivy).",
      "qualityGovernance": "Qualität & Governance",
      "scorecardGate": "Scorecard Hard Gate (70/100 erforderlich)",
      "scorecardGateBlurb": "Prüft Non-Root-Dockerfile, SAST/Container-Security-Scanning in CI und Kubernetes-Ressourcen-Requests/Limits.",
      "triggerPreview": "Initiale PR-Preview-Umgebung deployen",
      "triggerPreviewBlurb": "Stellt sofort nach Workspace-Erstellung eine aktive Preview-Bereitstellung bereit.",
      "creating": "Erstellen…",
      "gitlabConnectFirst": "GitLab unter Integrationen verbinden, bevor ein GitLab-Workspace erstellt wird.",
      "images": "Images",
      "loadTemplatesFailed": "Vorlagen konnten nicht geladen werden"
    },
    "detail": {
      "edit": "Katalog-Metadaten bearbeiten",
      "scorecard": "Scorecard",
      "save": "Änderungen speichern",
      "deleteTitle": "Aus Katalog entfernen?",
      "back": "← Katalog",
      "noDescription": "Keine Beschreibung",
      "openWorkspace": "Workspace öffnen",
      "updateService": "Katalogeintrag aktualisieren",
      "serviceUpdated": "Katalogeintrag aktualisiert",
      "ownerLabel": "Owner",
      "tierSlo": "Tier / SLO",
      "templateLabel": "Vorlage",
      "runbookLabel": "Runbook",
      "gateLabel": "Gate {gate} - {status}",
      "gatePassed": "bestanden",
      "gateNeedsWork": "Verbesserung nötig",
      "scoreOf": "Score {score}/100",
      "deleteConfirm": "\"{name}\" aus Markierte Workspaces entfernen? Entfernt nur den Katalog-Favoriten. Der verknüpfte Workspace bleibt erhalten, sofern nicht separat zerstört."
    },
    "errors": {
      "load": "Katalog konnte nicht geladen werden",
      "create": "Workspace konnte nicht erstellt werden",
      "update": "Katalogeintrag konnte nicht aktualisiert werden",
      "delete": "Katalog-Favorit konnte nicht entfernt werden"
    }
  },
  "org": {
    "title": "Organisation",
    "settings": "Organisationseinstellungen",
    "blurb": "Mitglieder, Einladungen und SSO-Gruppenzuordnungen verwalten.",
    "members": "Mitglieder",
    "inviteEmail": "Per E-Mail einladen",
    "sendInvite": "Einladung senden",
    "sending": "Senden…",
    "revoke": "Widerrufen",
    "ssoTitle": "SSO-Gruppe → Rolle",
    "ssoBlurb": "IdP-Gruppen auf Launchpad-Rollen abbilden.",
    "groupName": "Gruppenname",
    "saveMapping": "Zuordnung speichern",
    "remove": "Entfernen",
    "blurbDetail": "Mitglieder, E-Mail-Einladungen und SSO-Gruppe → Rolle-Zuordnungen für die aktive Organisation.",
    "inviteEmailed": "Einladung per E-Mail an {email} gesendet",
    "inviteEmailFailed": "Einladung für {email} erstellt, E-Mail fehlgeschlagen ({error}). Link: {url}",
    "inviteCreatedSmtp": "Einladung erstellt (SMTP nicht gesetzt). Teilen: {url}",
    "inviteCreatedFor": "Einladung erstellt für {email}",
    "inviteExpires": "läuft ab",
    "roleUpdated": "{email} auf {role} aktualisiert",
    "ssoBlurbDetail": "IdP-Gruppenclaims (aus OIDC_GROUP_CLAIM) auf Org-Rollen abbilden. Bei SSO-Anmeldung gewähren passende Gruppen Mitgliedschaft oder Beförderung.",
    "errors": {
      "load": "Organisation konnte nicht geladen werden",
      "invite": "Einladung fehlgeschlagen",
      "revoke": "Widerruf fehlgeschlagen",
      "mapping": "Zuordnung speichern fehlgeschlagen",
      "deleteMapping": "Zuordnung löschen fehlgeschlagen",
      "delete": "Löschen fehlgeschlagen"
    }
  },
  "terminal": {
    "title": "Sandbox-Terminal",
    "clear": "Leeren",
    "restart": "Neustart",
    "kill": "Beenden",
    "ariaLabel": "Interaktives Sandbox-Terminal",
    "status": {
      "idle": "inaktiv",
      "connecting": "verbinden…",
      "error": "Fehler",
      "killed": "beendet",
      "connected": "verbunden ({mode}) - tippen zum Interagieren"
    },
    "notConnected": "Terminal nicht verbunden",
    "restarting": "Sitzung wird neu gestartet…"
  },
  "envConsole": {
    "title": "Terminal & Observability",
    "blurb": "Shell in den Preview-Pod oder die VM, oder live Provisioning-Logs. CPU/Speicher und Health-Ping kommen aus der Control Plane.",
    "shell": "Shell",
    "logs": "Live-Logs",
    "refresh": "Metriken aktualisieren",
    "openObservability": "Observability",
    "cpu": "CPU",
    "memory": "Speicher",
    "healthPing": "Health-Ping",
    "healthy": "Gesund",
    "unhealthy": "Ungesund",
    "live": "live",
    "shellBanner": "Environment-Shell wird verbunden…",
    "logsBanner": "Environment-Logs werden gestreamt…",
    "status": {
      "idle": "inaktiv",
      "connecting": "verbinden…",
      "connected": "verbunden ({mode}) · {target}",
      "error": "Fehler",
      "disconnected": "getrennt",
      "streamingLogs": "Logs werden gestreamt…",
      "logsDone": "Log-Stream beendet",
      "logsError": "Log-Stream-Fehler"
    }
  },
  "envObservability": {
    "title": "Cluster Observability",
    "blurb": "Health und aggregierte Metriken für {name}",
    "back": "Zurück zur Environment",
    "window": "Letzte 1h",
    "autoRefresh": "Auto-Refresh",
    "loadFailed": "Observability-Daten konnten nicht geladen werden",
    "cards": {
      "cpu": "CPU (Sample)",
      "cpuHint": "{pct}% der gesampelten Kapazität",
      "sampleHint": "Aus Control-Plane-Sample",
      "errorRate": "Preview-Fehlerrate",
      "errorHint": "Basierend auf dem letzten Health-Ping",
      "latency": "Health-Latenz",
      "latencyHint": "Round-Trip zur Preview-URL",
      "alerts": "Aktive Alerts",
      "previewDown": "Preview-Health-Check fehlgeschlagen",
      "noAlerts": "Keine aktiven Alerts"
    },
    "signals": {
      "title": "Golden Signals der Core Services",
      "service": "Service",
      "status": "Status",
      "cpu": "CPU",
      "memory": "Speicher",
      "health": "Health"
    },
    "trace": {
      "title": "Aktuelle Samples"
    },
    "stream": {
      "title": "Aggregierter Stream"
    },
    "context": {
      "title": "Environment-Kontext",
      "target": "Ziel",
      "owner": "Owner",
      "provider": "Provider",
      "deploy": "Deploy-Modus",
      "appReady": "App bereit"
    }
  },
  "credentials": {
    "fields": {
      "serviceAccountJson": "Service-Account-JSON",
      "keylessWif": "Keyless (WIF / OIDC)",
      "gcpSaKeyJson": "GCP-SA-Key-JSON",
      "gcpSaKeyJsonPlaceholder": "Vollständiges heruntergeladenes Key-JSON einfügen (service_account Key-Objekt)",
      "gcpSaKeyHowTitle": "JSON aus einer Service-Account-E-Mail erzeugen",
      "gcpSaKeyHowIntro": "Eine E-Mail wie deploy{'@'}mein-projekt.iam.gserviceaccount.com ist nur die Identität. Sie müssen einen Key erstellen; Google liefert die JSON-Datei.",
      "gcpSaKeyHowStep1": "Google Cloud Console → IAM & Admin → Service Accounts für Ihr Projekt öffnen.",
      "gcpSaKeyHowStep2": "Service Account wählen (z. B. deploy{'@'}….iam.gserviceaccount.com).",
      "gcpSaKeyHowStep3": "Keys → Add key → Create new key → JSON. Eine .json-Datei wird heruntergeladen.",
      "gcpSaKeyHowStep4": "Datei öffnen, alles kopieren und hier einfügen. Nicht nur die E-Mail einfügen.",
      "gcpSaKeyHowCli": "gcloud iam service-accounts keys create ./sa.json --iam-account=deploy{'@'}PROJECT.iam.gserviceaccount.com --project=PROJECT",
      "gcpSaKeyHowNote": "Dem SA Compute-Rollen auf dem Projekt geben (z. B. roles/compute.instanceAdmin.v1). JSON wie ein Passwort behandeln.",
      "gcpProjectId": "GCP-Projekt-ID",
      "gcpProjectIdHint": "Erforderlich bei Connect Google. Service-Account-JSON enthält bereits project_id.",
      "gcpProjectIdFromSa": "Projekt-ID aus SA-JSON: {project}",
      "preferredRegion": "Bevorzugte Region",
      "preferredRegionPlaceholder": "Standardregion wählen",
      "preferredRegionHint": "Wird beim Cloud-Deploy als Region vorausgefüllt.",
      "gcpProjectNumber": "GCP-Projektnummer",
      "wifPoolId": "WIF-Pool-ID",
      "wifProviderId": "WIF-Provider-ID",
      "targetSaEmail": "Ziel-SA-E-Mail",
      "accessKeys": "Access Keys",
      "keylessOidc": "Keyless (Role ARN / OIDC)",
      "accessKeyId": "Access Key ID",
      "secretAccessKey": "Secret Access Key",
      "sessionTokenOptional": "Session Token (optional)",
      "iamRoleArn": "IAM-Role-ARN",
      "roleSessionNameOptional": "Role-Session-Name (optional)",
      "clientId": "Client ID",
      "clientSecret": "Client Secret",
      "tenantId": "Tenant ID",
      "subscriptionId": "Subscription ID",
      "apiToken": "API-Token",
      "overrideHint": "Optional. Gespeicherte Einstellungs-Schluessel werden genutzt, sofern Sie hier nichts einfuegen, um sie nur fuer diesen Workspace zu ueberschreiben.",
      "overrideToggle": "Cloud-Schluessel ueberschreiben (optional)",
      "showSaHelp": "JSON aus einem Service Account erzeugen",
      "hideSaHelp": "Service-Account-Hilfe ausblenden"
    }
  },
  "preview": {
    "live": "Live-Preview",
    "status": "Status",
    "timeLeft": "Verbleibende Zeit",
    "costToDate": "Kosten bisher",
    "rate": "Rate",
    "yourPreviewLive": "Deine Preview ist live",
    "openApp": "App öffnen",
    "provisioning": "Wird bereitgestellt…",
    "hangTight": "Einen Moment - diese Seite aktualisiert sich automatisch.",
    "unavailable": "Preview nicht verfügbar",
    "loading": "Preview wird geladen…",
    "openInLaunchpad": "In Launchpad öffnen",
    "prPreview": "PR-Preview",
    "copyLink": "Link kopieren",
    "linkCopied": "Link kopiert",
    "linkCopiedBody": "Teilbarer Status-Link ist in der Zwischenablage.",
    "copyFailed": "Kopieren fehlgeschlagen",
    "copyFailedBody": "Link konnte nicht kopiert werden - aus der Adresszeile kopieren.",
    "notFound": "Preview nicht gefunden",
    "envDetails": "Umgebungsdetails →",
    "shareableBlurb": "Dies ist die teilbare Statusseite. Open app für die laufende Workload-URL nutzen.",
    "viewLogs": "Logs anzeigen",
    "statusIs": "Status ist {status}.",
    "prTitle": "Stabile URL für PR #{number}",
    "prBlurb": "Dieser Pfad ist der stabile Launchpad-PR-Preview-Link. Beim Schließen des PR wird die Umgebung automatisch abgebaut.",
    "resolving": "Preview wird aufgelöst…",
    "noPreview": "Keine aktive Preview mit diesem PR verknüpft. Eine starten unter",
    "noPreviewSuffix": "mit gesetzter PR-Nummer.",
    "resolveFailed": "PR-Preview konnte nicht aufgelöst werden"
  },
  "docs": {
    "title": "Dokumentation",
    "blurb": "Launchpad nutzen und betreiben.",
    "guide": "Leitfaden",
    "pageTitle": "Launchpad nutzen",
    "pageBlurb": "Launchpad ist ein internes Entwicklerportal für gesteuerte ephemere App-Umgebungen und Multi-Cloud-Infrastruktur. Dieser Leitfaden erklärt, wie das Produkt funktioniert, und führt Schritt für Schritt durch jeden Flow.",
    "howItWorks": "So funktioniert es",
    "gettingStarted": "Erste Schritte",
    "homeSettings": "Start & Einstellungen",
    "launchEnvironment": "Umgebung starten",
    "gitPushRebuilds": "Git-Push-Rebuilds",
    "applyManifests": "Kubernetes-Manifeste anwenden",
    "networkTopology": "Netzwerktopologie",
    "githubConnect": "GitHub Connect",
    "teardownTtl": "Abbau & TTL",
    "workspacesTerminal": "Workspaces & Sandbox-Terminal",
    "hybridCloud": "Fleet",
    "catalog": "Service-Katalog",
    "projects": "Projekte",
    "organizationGuide": "Organisation & Zugriff",
    "integrationsGuide": "Integrationen-Hub",
    "environmentOps": "Umgebungs-Operationen",
    "dockerfilesGuide": "Dockerfiles",
    "authAccess": "Anmeldung, SSO & Einladungen",
    "provisionExtras": "Ansible, Kustomize & Scaffolds",
    "launch": "Launch",
    "provision": "Provision",
    "hybrid": "Fleet",
    "catalogNav": "Katalog",
    "projectsNav": "Projekte",
    "orgNav": "Organisation",
    "integrationsNav": "Integrationen",
    "dockerfilesNav": "Dockerfiles",
    "provisionCloudInfra": "Cloud-Infrastruktur provisionieren",
    "ansibleEngine": "Ansible",
    "kustomizeMode": "Kustomize",
    "pulumiEngine": "Pulumi",
    "terraformEngine": "Terraform",
    "productGuidePath": "docs/product-guide.md",
    "ssoLogin": "SSO",
    "moreGuides": "Siehe auch {productGuide} im Repo für die vollständige Feature-Karte.",
    "workspaces": "Workspaces",
    "environments": "Umgebungen",
    "integrations": "Integrationen",
    "organization": "Organisation",
    "settings": "Einstellungen",
    "cloudCredentials": "Cloud-Zugangsdaten",
    "home": "Start",
    "devLogin": "Dev-Login",
    "localSandbox": "Lokal (Sandbox)",
    "running": "RUNNING",
    "openPreview": "Preview öffnen",
    "manifestDeploy": "Manifest-Deploy",
    "rawK8sManifests": "Raw K8s Manifests",
    "helm": "Helm",
    "generateWorkspace": "Workspace erzeugen",
    "pushToGithub": "Push to GitHub",
    "kubectlApplyDelete": "kubectl apply / delete",
    "planApplyDestroy": "plan / apply / destroy",
    "connectGithub": "GitHub verbinden",
    "simple": "Einfach",
    "standard": "Standard",
    "accountSettings": "Kontoeinstellungen",
    "perWorkspace": "Pro Workspace",
    "administration": "Administration",
    "contents": "Contents",
    "secrets": "Secrets",
    "metadata": "Metadata",
    "accept": "Accept",
    "personalAccount": "persönliches Konto",
    "gcpSaKeyJson": "GCP-SA-Key-JSON",
    "runKubectlOnOpen": "kubectl apply in Sandbox beim Öffnen ausführen",
    "provisionInfraLabel": "Provision (Infra)",
    "manifestWorkloadLabel": "Manifest (Workload)",
    "environmentPreviewLabel": "Environment (Preview)",
    "integrationsGithub": "Integrationen → GitHub",
    "cloudCredentialsLink": "Cloud-Zugangsdaten",
    "githubApp": "GitHub App",
    "overview": {
      "intro": "Launchpad hat vier verwandte Aufgaben. Sie teilen sich dieselbe UI und können verknüpft werden, laufen aber mit unterschiedlichen Lebenszyklen:",
      "provisionBody": "Terraform- oder Pulumi-Workspace für GCP, AWS, Azure, Cloudflare oder kind erzeugen. Sie hängen kurzlebige Cloud-Zugangsdaten an, wählen Ressourcen und wenden den Stack im Sandbox-Terminal an. Damit stellen Sie Plattform-Infrastruktur bereit (VPC, Cluster usw.).",
      "manifestBody": "Kubernetes-YAML oder Helm-Charts aus {path} in Ihrem Workspace anwenden. Damit deployen Sie App-Objekte auf einen bereits vorhandenen Cluster. Im Sandbox-Terminal werden Manifeste getrennt nach Terraform/Pulumi angewendet.",
      "environmentBody": "kurzlebige, gesteuerte Preview einer App aus Git-Repo und Branch. Launchpad kann Kubernetes-Previews, Docker Compose oder eine Cloud-/VM-Running-Instance (attach) betreiben. Im Instance-Modus klont Launchpad das verknüpfte App-Repo auf den Host (oder synct den Workspace), startet den Prozess, streamt Status und Logs und räumt alles ab, wenn die TTL abläuft. Bei Verknüpfung mit einem Workspace mit Raw-Manifesten nutzen Umgebungen {manifestDeploy} statt des eingebauten Preview-Profils.",
      "hybridBody": "selbst gehostete Linux-Hosts als Agent-Knoten einschreiben (ausgehender sicherer Tunnel) und optional abgesicherte Infrastruktur-Blueprints mit KI erzeugen. Container auf einem Homelab-Knoten deployen oder den Blueprint auf GCP / AWS / Azure abbilden.",
      "useFlows": "Nutzen Sie {launch} für eine One-Click-Preview - Local Sandbox (ein Bildschirm) oder Cloud, Katalogvorlage oder eigenes Repo. Verwalten Sie laufende Previews über {environments}. Erstellen Sie einen neuen Cloud-Stack über {provision}, öffnen Sie bereits erzeugte Workspaces über {workspaces}, und verwalten Sie Homelab-Knoten sowie KI-Blueprints über {hybrid}."
    },
    "gettingStartedSteps": {
      "step1": "Öffnen Sie Launchpad im Browser.",
      "step2": "Melden Sie sich mit Ihrem Konto an oder nutzen Sie {devLogin}, wenn diese Option verfügbar ist.",
      "step3": "Sie landen im {home}-Hub - von dort öffnen Sie Umgebungen, Provision oder Einstellungen. Die öffentliche Produktseite ist {root}.",
      "step4": "Optional GCP-/AWS-/Azure-/Cloudflare-Schlüssel unter {settings} hinterlegen, damit Workspaces sie wiederverwenden können."
    },
    "homeSettingsSection": {
      "body": "{home} zeigt aktive Umgebungs- und Workspace-Zahlen sowie, ob Cloud-Schlüssel des Kontos konfiguriert sind. {settings} speichert verschlüsselte Cloud-Zugangsdaten für Ihren Benutzer (SA-JSON, WIF, AWS-Keys/Rollen, Azure-SP, Cloudflare-Token). Leere Felder in Provision greifen auf diesen Tresor zurück."
    },
    "environmentsSection": {
      "intro": "Eine Umgebung ist eine gesteuerte, zeitlich begrenzte Preview Ihrer Anwendung. Der schnellste Weg ist {launch} - wählen Sie {localSandbox}, um lokal ohne Cloud-Zugangsdaten zu testen, oder verbinden Sie GCP/AWS/Azure/Cloudflare. Bei Running-Instance-Workspaces provisioniert Launchpad die VM (oder das Ziel), liefert das verknüpfte GitHub-/GitLab-Repo aus, und Open app zeigt auf den laufenden Prozess (kein Docker-Image nötig, außer Sie wählen eine Docker-Strategie). Die Bereitstellung läuft asynchron; die Detailseite zeigt Live-Status und Logs während des Hochfahrens.",
      "step1": "Öffnen Sie {launchPath} und wählen Sie ein Ziel (Local Sandbox ist der Standard für lokale Tests).",
      "step2": "Für lokal: {kindCmd} ausführen (mappt NodePorts auf localhost), {k8sEnabled} und {k8sContext} in der API-{envFile} setzen, dann API und Worker neu starten. Preview öffnen erreicht den echten Pod unter {previewUrl}.",
      "step3": "Preview-App-Vorlage wählen, Umgebung benennen und starten.",
      "step4": "Umgebung öffnen, Live-Logs verfolgen, {running} bestätigen und {openPreview} nutzen."
    },
    "rebuildSection": {
      "intro": "Wenn ein GitHub- oder GitLab-Webhook konfiguriert ist, löst ein Push auf einen Branch, der zu Repo und Branch einer aktiven Umgebung passt, automatisch einen Rebuild aus. Instance-Umgebungen klonen (oder synchen) die verknüpfte App erneut und starten den Prozess auf der VM neu. Die Umgebungsdetailseite zeigt, ob {webhookSecret} in der API gesetzt ist (Plattform-Secret, muss zum GitHub-App-Webhook-Secret passen).",
      "step1": "Umgebung gegen den Branch laufen lassen, an dem Sie iterieren.",
      "step2": "Commits auf diesen Branch in GitHub oder GitLab pushen.",
      "step3": "Launchpad markiert die Umgebung erneut als provisioning, speichert den neuesten Commit und deployt neu (K8s-Apply, Compose up oder VM-Clone/Neustart).",
      "step4": "Umgebungskarte oder Detailseite beobachten - Status, Commit-SHA und Logs aktualisieren sich live. Workload-Image erscheint nur bei einem echten Container-Image; Instance-/Source-Deploys zeigen keines."
    },
    "provisionSection": {
      "intro": "Der Provision-Assistent erzeugt ein IaC-Bundle für Sie. Bei klassischen Cluster-Stacks führen Sie plan/apply meist im Sandbox-Terminal aus, nachdem das Bundle bereit ist. Bei Running-Instance-(attach)-Cloud-Zielen kann Launchpad den VM-Stack anwenden und das verknüpfte App-Repo per SSH/Clone ausliefern, sodass Open app Ihr echtes Frontend oder Ihre API bedient.",
      "step1": "Gehen Sie zu {provision}.",
      "step2": "Workspace-Namen, Cloud-Anbieter und IaC-Engine ({terraform}, {pulumi} oder {ansible}) wählen.",
      "step3": "Kurzlebige Cloud-Zugangsdaten für diesen Anbieter einfügen (siehe {credentialsLink}).",
      "step4": "Ressourcen für den erzeugten Stack wählen (VPC, Cluster, Storage usw.).",
      "step5": "Wenn Sie einen Cluster aktivieren (GKE, EKS, AKS, …), wählen Sie {rawK8s}, {helm} oder {kustomize} und schalten Sie die Kubernetes-Objekte zum Scaffolding ein - Deployment, Service, Ingress, Pod, Job, CronJob, StatefulSet, DaemonSet, ConfigMap, Secret, PVC, Role, HPA und mehr.",
      "step6": "Optional GitHub verbinden und ein Repository mit CI-Workflow und Cloud-Secrets bootstrappen.",
      "step7": "Mit {generateWorkspace} abschließen. Launchpad scaffoldet die IaC-Dateien und öffnet ein Sandbox-Terminal.",
      "step8": "Im Terminal zuerst Infrastruktur anwenden: {tfPlan} / {tfApply} oder {pulumiUp}. Manifest-Apply ({kubectlApply} / {helmUpgrade}) ist ein separater Schritt danach.",
      "credentialsNote": "Zugangsdaten werden verschlüsselt gespeichert, für diese Sitzung in die Sandbox injiziert und nie im Klartext in Logs angezeigt."
    },
    "manifestSection": {
      "intro": "Manifest-Deploy ist getrennt von Cloud-Infra-Provision. Workspaces mit {rawK8s} enthalten Dateien unter {path}. Sie können sie manuell im Sandbox-Terminal anwenden oder Launchpad beim Start einer verknüpften Umgebung anwenden lassen.",
      "step1": "Provision-Assistent abschließen und Raw-K8s-Manifests-Packaging aktivieren.",
      "step2": "Workspace-Terminal mit {runKubectlOnOpen} öffnen, um Manifeste auf den Cluster anzuwenden, oder zuerst Infra und dann Manifeste manuell anwenden.",
      "step3": "Für eine gesteuerte Preview aus diesen Manifesten {launch} öffnen und den Workspace wählen - Launchpad nutzt automatisch {manifestDeploy}."
    },
    "networkSection": {
      "intro": "Wenn VPC/VNet und Subnets aktiviert sind, wählen Sie in Provision eine Topologie:",
      "simpleBody": "ein Subnetz (plus Internet-Gateway bei AWS). Am besten für Demos und ephemere Stacks. CIDRs werden pro Umgebung automatisch erzeugt, damit parallele Stacks nicht kollidieren.",
      "standardBody": "öffentliche und private Subnetze mit verwaltetem NAT-Egress (Cloud NAT bei GCP, NAT Gateway bei AWS/Azure). Cluster hängen am privaten Subnetz. Bevorzugen Sie dies für produktionsnahe Golden Paths."
    },
    "credentialsSection": {
      "intro": "Zugangsdaten können an zwei Stellen gespeichert werden:",
      "accountSettingsBody": "verschlüsselter Tresor für GCP (SA-JSON oder WIF), AWS (Keys oder Role-ARN), Azure-SP und Cloudflare. Fallback, wenn Workspace-Felder leer sind.",
      "perWorkspaceBody": "im Provision-Assistenten - überschreibt oder füllt nur diesen Workspace.",
      "preferShortLived": "Bevorzugen Sie kurzlebige Zugangsdaten oder keyless OIDC. Nutzen Sie least privilege für die gewählten Ressourcen."
    },
    "gcpCredentials": {
      "title": "GCP - Service-Account-JSON",
      "step1": "Öffnen Sie {iamLink} in der Google Cloud Console.",
      "step2": "Service Account im Zielprojekt auswählen oder anlegen.",
      "step3": "Rollen für die zu provisionierenden Ressourcen vergeben (z. B. Compute Admin, Kubernetes Engine Admin, Secret Manager Admin).",
      "step4": "Keys → Add key → Create new key → JSON - Datei herunterladen.",
      "step5": "In Provision das vollständige JSON in {gcpSaKeyJson} einfügen und die Project ID setzen.",
      "iamLinkLabel": "IAM → Service Accounts"
    },
    "awsCredentials": {
      "title": "AWS - Access Keys",
      "step1": "Melden Sie sich in der {iamConsole} an.",
      "step2": "Benutzer mit Least-Privilege-Richtlinien für benötigte Ressourcen anlegen oder auswählen (VPC, EKS, S3 usw.).",
      "step3": "Security credentials → Create access key → Command Line Interface (CLI).",
      "step4": "Access key ID und Secret access key in Provision einfügen. Session Token hinzufügen, wenn Sie temporäre STS-Zugangsdaten nutzen.",
      "iamConsoleLabel": "AWS-IAM-Konsole"
    },
    "azureCredentials": {
      "title": "Azure - Service Principal",
      "step1": "Azure Portal → Microsoft Entra ID → App registrations → New registration.",
      "step2": "Certificates & secrets → New client secret - Wert sofort kopieren.",
      "step3": "Application (client) ID, Directory (tenant) ID und Subscription ID notieren.",
      "step4": "Der App eine Rolle auf Subscription oder Resource Group zuweisen (z. B. Contributor).",
      "step5": "Client ID, Client secret, Tenant ID, Subscription ID und Resource group in Provision einfügen."
    },
    "cloudflareCredentials": {
      "title": "Cloudflare - API-Token",
      "step1": "Öffnen Sie {apiTokensLink}.",
      "step2": "Token mit Berechtigungen für Workers, R2 und/oder DNS nach Bedarf erstellen.",
      "step3": "Token und Account ID in Provision einfügen.",
      "apiTokensLinkLabel": "My Profile → API Tokens"
    },
    "githubSection": {
      "intro": "Launchpad nutzt eine {githubApp}, damit Sie kein Personal Access Token im Browser einfügen müssen. Beim Verbinden installieren oder autorisieren Sie die App auf Ihrem Benutzer- oder Organisationskonto; Launchpad legt dann Repos an und setzt CI-Secrets während Provision in Ihrem Namen.",
      "step1": "Öffnen Sie {integrationsGithub}.",
      "step2": "Klicken Sie auf {connectGithub}.",
      "step3": "Wählen Sie auf GitHub Konto oder Organisation und die Repositories, auf die die App zugreifen darf, dann Authorize / Install.",
      "step4": "Nach Abschluss der Installation werden Sie zurück zu Launchpad geleitet.",
      "step5": "Prüfen Sie, dass Installationen auf der GitHub-Seite erscheinen - wählen Sie eine beim Bootstrappen eines Repos im Provision-Assistenten.",
      "appNeedsTitle": "Was die App braucht",
      "permAdministration": "Repositories anlegen",
      "permContents": "Workflow- und Infrastrukturdateien pushen",
      "permSecrets": "CI-Cloud-Secrets setzen",
      "permMetadata": "nur lesen",
      "afterPermissions": "Nach Berechtigungsänderungen die Installation auf GitHub öffnen und {accept} für die neue Anfrage. Apps auf einem {personalAccount} können per API keine neuen Repos anlegen - legen Sie zuerst ein leeres Repo an oder installieren Sie auf einer {organization}."
    },
    "workspacesSection": {
      "intro": "Jeder abgeschlossene Provision-Lauf wird ein Workspace: erzeugte Terraform- oder Pulumi-Dateien plus Sandbox, in der Ihre Cloud-Zugangsdaten bereits verfügbar sind. Verknüpfen Sie ein GitHub- oder GitLab-App-Repo am Workspace, damit Cloud-Instance-Umgebungen diesen Code standardmäßig klonen und ausführen (Import/Scaffold kann den Workspace weiterhin per SSH syncen). Die Workspace-Seite enthält einen IDE-Explorer zum Bearbeiten von Manifesten, Hinzufügen von Kubernetes-/Terraform-Vorlagen, Speichern, Formatieren, Push to GitHub und kubectl-/terraform-Befehle im Terminal.",
      "step1": "Öffnen Sie {workspaces} und wählen Sie einen Workspace.",
      "step2": "Explorer nutzen, um Dateien zu erstellen, umbenennen, löschen, bearbeiten, formatieren und speichern ({saveShortcut}).",
      "step3": "Kubernetes-YAML (Deployment, Service, Ingress, Pod, Job, …) oder Terraform-Stubs aus den Vorlagenmenüs hinzufügen.",
      "step4": "Auf {pushToGithub} klicken, um das aktuelle Bundle in ein verbundenes Repository zu committen.",
      "step5": "{kubectlApplyDelete} oder Terraform {planApplyDestroy} nutzen - Befehle gehen ans Sandbox-Terminal."
    },
    "teardownSection": {
      "intro": "Umgebungen sind temporär gedacht. Launchpad trackt die verbleibende Laufzeit jeder Umgebung und räumt abgelaufene automatisch auf.",
      "step1": "Für frühen Abbau die Umgebung öffnen und {destroy} klicken (oder vom Dashboard zerstören).",
      "step2": "Status wechselt während des Abbaus, während Ressourcen entfernt werden; Live-Logs beobachten.",
      "step3": "Ohne Aktion läuft die Umgebung ab, wenn die TTL erreicht ist, und Launchpad räumt sie planmäßig auf."
    },
    "hybridCloudLabel": "Fleet (Knoten + KI-Deploy)",
    "hybridSection": {
      "intro": "Fleet verbindet selbst gehostete Hosts und Cloud-Ziele. Öffnen Sie {hybridPath}, um Agent-Knoten einzuschreiben und den KI-Infrastruktur-Provisioner zu nutzen. Ausführliche Referenz: {docPath}.",
      "whatTitle": "Was Sie bekommen",
      "whatAgents": "Agent-Knoten: Linux-Hosts, die einen ausgehenden sicheren Tunnel zu Launchpad aufbauen.",
      "whatAi": "KI-Infrastruktur-Provisioner: Alltagssprache zu einem abgesicherten Container-Blueprint mit Kostenschätzung.",
      "whatTargets": "Deploy-Ziele: ein online selbst gehosteter Knoten (Docker über den Tunnel) oder GCP / AWS / Azure über den bestehenden Provision-Pfad.",
      "enrollTitle": "Knoten einschreiben",
      "enroll1": "{hybridPath} öffnen und Knoten einschreiben klicken. Host benennen (z. B. homelab-nas).",
      "enroll2": "Einzeiligen Installationsbefehl kopieren und auf dem Host ausführen (Docker erforderlich).",
      "enroll3": "Warten, bis der Knoten Online mit aktuellem Zuletzt gesehen zeigt.",
      "enroll4": "Widerrufen entfernt den Knoten aus Launchpad und trennt den Agenten.",
      "aiTitle": "KI-Deploy",
      "ai1": "Gewünschten Stack in Alltagssprache beschreiben.",
      "ai2": "Selbst gehosteten Knoten (online wählen) oder Cloud-Anbieter wählen, dann Blueprint erzeugen.",
      "ai3": "Services und Kosten prüfen, deployen, dann den Ansichtslink öffnen, um das Ergebnis zu sehen.",
      "securityTitle": "Sicherheitshinweise",
      "securityBody": "Einschreibe-Token sind einmalig und kurzlebig. Agenten authentifizieren den Reverse-Tunnel mit HMAC-Geheimnissen pro Knoten. Docker-Zugriff auf dem Host möglichst restriktiv halten."
    },
    "catalogSection": {
      "intro": "Der {catalog} enthält Golden-Path-Vorlagen und markierte Workspaces für Launch.",
      "step1": "{catalogPath} öffnen. Unter Templates fertige Stacks durchsuchen.",
      "step2": "Im Tab Starred Workspaces anzeigen, die Sie in Workspaces favorisiert haben; Markierung entfernen, wenn fertig.",
      "step3": "Unter {launch} eine Katalogvorlage wählen statt einer rohen Git-URL, wenn Sie einen bekannten Starter wollen."
    },
    "projectsSection": {
      "intro": "Projekte gruppieren Workspaces in Ihrer aktiven Organisation und setzen Planlimits durch.",
      "step1": "Aktive Org in der Shell prüfen, dann {projectsPath} öffnen.",
      "step2": "Owner und Admins legen ein Projekt per Namen an (zählt gegen den Plan).",
      "step3": "Im Projekt: Mitglieder und Einladungen verwalten, Git-Repo importieren oder Provision / Workspaces für dieses Projekt öffnen."
    },
    "orgSection": {
      "intro": "{orgPath} ist der Ort, an dem Admins Zugriff und Abrechnung der aktiven Org verwalten.",
      "members": "Personen per E-Mail und Rolle einladen, Rollen ändern und ausstehende Einladungen widerrufen. Eingeladene öffnen den Link unter /invite/….",
      "sso": "IdP-Gruppen-Claims auf Org-Rollen für OIDC-SSO mappen. Passende Gruppen erzeugen oder fördern Mitgliedschaft beim Login (Owner werden durch SSO nie herabgestuft).",
      "billing": "Wenn Stripe konfiguriert ist, Plan-Nutzung anzeigen und Checkout oder Kundenportal unter Organisation öffnen."
    },
    "integrationsSection": {
      "intro": "{integrationsPath} ist der Hub für Quellcode- und Kollaborations-Connectoren.",
      "github": "GitHub App - Repos anlegen, CI pushen, Webhooks, PR-Kommentare und Commit-Status (siehe GitHub Connect unten).",
      "gitlab": "GitLab - OAuth oder Personal Access Token zum Listen von Projekten, Anlegen von Repos und Pushen gespiegelter Dateien.",
      "slack": "Slack - Org Incoming Webhook für Ready, Failed, TTL-Warnung und Soft-Cost-Cap-Alerts.",
      "jira": "Jira Cloud - E-Mail + API-Token + Site-URL; automatische oder manuelle Issues bei Provision-/Rebuild-Fehler. Issues von der Umgebungsdetailseite verlinken."
    },
    "environmentOpsSection": {
      "intro": "Unter {environmentsPath} und auf jeder Umgebungsdetailseite können Sie Previews nach dem Start betreiben.",
      "actions": "TTL verlängern, Pause/Resume, fehlgeschlagenes Provision erneut versuchen, Deploy to cloud (nur lokales Staging/Production), Drift scannen, Preview-Analyzer ausführen und Jira anlegen/öffnen wenn verbunden.",
      "share": "Fortschritt über die Statusseite /p/:id teilen. GitHub-PR über /pr/:number auflösen, wenn Launchpad diesen PR getrackt hat.",
      "caps": "Gleichzeitige Previews und Soft-Cost-Caps gelten. Die Benachrichtigungsglocke im Header zeigt Lifecycle-Ereignisse. Soft-Cost-Cap kann auch Slack benachrichtigen."
    },
    "dockerfilesSection": {
      "intro": "{dockerfilesPath} hilft, wenn ein Repo ein Dockerfile für Preview-Builds braucht.",
      "step1": "GitHub verbinden und ein Repository auf vorhandene Dockerfiles und Stack-Hinweise scannen.",
      "step2": "Dockerfile scaffolden oder verbessern; mit Heuristik oder Gemini prüfen, wenn konfiguriert.",
      "step3": "Scaffold zurück ins Repo pushen und/oder einen Registry-Build starten und den Job-Status beobachten."
    },
    "authSection": {
      "intro": "Launchpad unterstützt Passwort-Konten, optionales Dev-Login und OIDC-SSO.",
      "login": "Anmeldung unter {loginPath}. Bei aktiviertem SSO Mit SSO anmelden nutzen, um über den Firmen-IdP zu gehen.",
      "onboarding": "Ohne Organisation führt {onboardingPath} zur ersten Org vor dem Home.",
      "invites": "Org- und Projekt-Einladungen kommen als Links unter /invite/…. Angenommen werden, während Sie mit der eingeladenen E-Mail angemeldet sind."
    },
    "provisionExtrasSection": {
      "intro": "Neben Terraform und Pulumi können Provision und die Workspace-IDE erzeugen:",
      "ansible": "{ansible} - Inventory und Playbooks unter infra/ansible; Check/Apply aus der Workspace-Toolbar, wenn erkannt.",
      "kustomize": "{kustomize} - dritter Kubernetes-Packaging-Modus neben Raw-Manifesten und Helm.",
      "scaffolds": "Optionale CI (GitHub Actions / GitLab CI), Pipeline-Security-Schalter und kostenorientierte Optionen (Spot, HPA, VPA, Idle-Shutdown) im Bundle.",
      "import": "Bestehendes GitHub- oder GitLab-Repo aus Workspaces (?import=1) oder aus einem Projekt importieren."
    }
  },
  "audit": {
    "title": "Ausführungs-Pipeline",
    "empty": "Noch keine Audit-Ereignisse.",
    "loading": "Audit-Ereignisse werden geladen…",
    "loadingShort": "laden…",
    "eventsTotalOne": "1 Ereignis gesamt",
    "eventsTotal": "{count} Ereignisse gesamt",
    "live": "Live"
  },
  "k8s": {
    "banner": {
      "connected": "Verbunden:",
      "context": "Kontext:",
      "namespace": "ns:",
      "nodes": "{count} Nodes",
      "applyManifests": "Manifeste anwenden",
      "applyingPipeline": "Pipeline wird angewendet…",
      "refreshState": "Status aktualisieren",
      "nukeWorkspace": "Workspace löschen",
      "healthyDefault": "Gesund (100 %)",
      "providers": {
        "gcp": "GCP (GKE)",
        "aws": "AWS (EKS)",
        "azure": "Azure (AKS)",
        "local": "Lokal (Sandbox)"
      }
    },
    "suite": {
      "clusterWarning": "Cluster-Verbindungswarnung",
      "aiAnalyzeFix": "KI analysieren & beheben",
      "nukeTitle": "Diesen Workspace löschen?",
      "nukeMessage": "Dies zerstört dauerhaft den Workspace, reißt davon bereitgestellte Environments ab und entfernt von Launchpad verwaltete Cloud-Ressourcen (Namespaces, Registry-Images und den gemeinsamen Preview-Cluster, wenn ungenutzt).",
      "nukeConfirm": "Ja, Workspace löschen",
      "deleteResource": "Ressource löschen?",
      "deleteResourceMessage": "Führt kubectl delete im Namespace {namespace} aus. Dies kann nicht rückgängig gemacht werden.",
      "execPodOnly": "Exec ist nur für Pods verfügbar. Wähle einen Pod unter {name}, nicht {kind}."
    },
    "grid": {
      "allResources": "Alle Ressourcen ({count})",
      "deployments": "Deployments",
      "pods": "Pods",
      "services": "Services",
      "ingress": "Ingress",
      "configMaps": "ConfigMaps",
      "empty": "Keine Ressourcen für die gewählte Kategorie. Klicke \"Manifeste anwenden\", um Workloads auf den Cluster zu deployen.",
      "namespace": "Namespace:",
      "readyReplicas": "Bereite Replicas:",
      "ipAddress": "IP-Adresse:",
      "ports": "Ports:",
      "endpoint": "Endpoint:",
      "describe": "Describe",
      "describeTitle": "Ressource beschreiben",
      "logsTitle": "Pod-Logs anzeigen",
      "exec": "Exec",
      "execTitle": "Container-Terminal-Shell öffnen",
      "deleteTitle": "Ressource löschen"
    },
    "logs": {
      "title": "Pod-Logs: {name}",
      "meta": "Namespace: {namespace} | Container: {container}",
      "filterPlaceholder": "Logs filtern…",
      "autoScroll": "Auto-Scroll",
      "export": "Exportieren",
      "connecting": "Log-Stream wird verbunden…",
      "noMatches": "Keine passenden Log-Einträge gefunden.",
      "showingLines": "{count} Zeilen angezeigt",
      "liveConnected": "Live-SSE-Stream verbunden"
    },
    "describe": {
      "namespace": "Namespace: {namespace}",
      "yamlTab": "YAML-Definition",
      "eventsTab": "Ereignisverlauf ({count})",
      "copyYaml": "YAML kopieren",
      "loading": "kubectl-describe-Metadaten werden geladen…",
      "noYaml": "# Keine YAML-Definition gefunden",
      "noEvents": "Keine recenten Kubernetes-Control-Plane-Ereignisse.",
      "colType": "Typ",
      "colReason": "Grund",
      "colMessage": "Nachricht",
      "colAge": "Alter"
    },
    "exec": {
      "title": "Container-Shell: {name}",
      "subtitle": "Interaktive PTY-WebSocket (`kubectl exec -it`)",
      "connected": "Verbunden",
      "connecting": "Verbinden…"
    },
    "packaging": {
      "title": "Kubernetes-Packaging",
      "blurb": "Wähle, wie Workloads gescaffoldet werden, dann die YAML-Arten. Namespace wird bei aktiviertem Packaging immer geschrieben.",
      "objectsTitle": "Hinzuzufügende Kubernetes-Objekte",
      "objectsBlurb": "Schalte die Ressourcenarten ein, die als YAML (oder Helm-Templates) erzeugt werden sollen.",
      "ingressClass": "Ingress-Klasse",
      "none": "Keine",
      "noneDesc": "Workload-Manifeste überspringen",
      "rawManifests": "Rohe K8s-Manifeste",
      "rawManifestsDesc": "infra/k8s/manifests/",
      "helm": "Helm-Chart",
      "helmDesc": "infra/helm/app-chart/",
      "kustomize": "Kustomize",
      "kustomizeDesc": "infra/kustomize/",
      "groups": {
        "workloads": "Workloads & Netzwerk",
        "config": "Config, Identität & Storage",
        "policy": "Autoscaling & Policy"
      }
    }
  },
  "manifest": {
    "form": {
      "label": "Interface-Formular",
      "title": "Manifest- & CI-Dateien bearbeiten",
      "blurb": "Dateien unter infra/ und ci/ bearbeiten, ohne die Advanced IDE zu öffnen.",
      "saveChanges": "Änderungen speichern",
      "empty": "Noch keine Dateien unter infra/ oder ci/. Nutze die Aktionen oben, um Provision, Kubernetes oder CI/CD zu scaffolden.",
      "files": "Dateien",
      "loadingFile": "Datei wird geladen…",
      "selectFile": "Datei zum Bearbeiten auswählen."
    },
    "configurator": {
      "defaultTitle": "Strukturierter Infra-Editor",
      "mapped": "Gemappt",
      "ide": "IDE",
      "aiAnalyze": "KI analysieren",
      "saveChanges": "Änderungen speichern",
      "moreActions": "Weitere Dateiaktionen",
      "discard": "Verwerfen",
      "deleteFile": "Datei löschen",
      "deleting": "Löschen…",
      "selectSidebar": "Infra-Datei in der Seitenleiste wählen, um strukturierte Felder zu bearbeiten.",
      "loadingFile": "Datei wird geladen…",
      "loadingEditor": "Editor wird geladen…",
      "unsavedChanges": "Ungespeicherte Änderungen",
      "noPendingChanges": "Keine ausstehenden Änderungen",
      "deleteTitle": "Datei löschen?",
      "sections": {
        "namespace": "Namespace",
        "hpa": "Horizontal Pod Autoscaler",
        "vpa": "Vertical Pod Autoscaler",
        "pdb": "Pod Disruption Budget",
        "ingress": "Ingress",
        "configMap": "ConfigMap-Daten",
        "secret": "Secret-Daten",
        "serviceAccount": "ServiceAccount",
        "networkPolicy": "NetworkPolicy",
        "resourceQuota": "ResourceQuota",
        "limitRange": "LimitRange",
        "workloadIdentity": "Workload-Identität",
        "serviceLink": "Service ↔ Deployment-Verknüpfung",
        "appContainer": "App-Container",
        "containerConfig": "Container-Konfiguration",
        "networking": "Netzwerk",
        "containerPort": "Container-Port",
        "envVars": "Umgebungsvariablen",
        "provision": "Infrastruktur-Provisionierung",
        "cicd": "CI/CD-Pipeline"
      }
    },
    "explorer": {
      "title": "Explorer",
      "fileCountOne": "{count} Datei",
      "fileCountMany": "{count} Dateien",
      "restoreFiles": "Dateien wiederherstellen",
      "restoring": "Wiederherstellen…",
      "noFiles": "Keine Dateien im Workspace gefunden.",
      "hint": "Gemappte Datei wählen, um strukturierte Felder zu bearbeiten.",
      "refreshTitle": "Aktualisieren"
    }
  },
  "scaffold": {
    "infra": {
      "enable": "Aktivieren",
      "createFiles": "Dateien erstellen",
      "run": "Ausführen",
      "copyYaml": "YAML kopieren",
      "copyDockerfile": "Dockerfile kopieren",
      "provision": {
        "label": "Provision",
        "title": "Provision erstellen",
        "blurb": "Terraform- oder Pulumi-Dateien unter infra/ scaffolden.",
        "localHint": "Lokale Kubernetes-Workspaces nutzen nur Manifeste."
      },
      "kubernetes": {
        "label": "Kubernetes",
        "title": "Kubernetes erstellen",
        "blurb": "Rohe Manifeste, Helm-Chart oder Kustomize-Layout unter infra/ scaffolden.",
        "runtimeHint": "Zuerst einen Kubernetes-Cluster-Service aktivieren (GKE, EKS oder AKS).",
        "modeK8s": "K8s-Manifeste",
        "modeHelm": "Helm-Chart",
        "modeKustomize": "Kustomize"
      },
      "cicd": {
        "label": "CI/CD",
        "title": "CI/CD erstellen",
        "blurb": "Multi-Job-GitHub- oder GitLab-Pipeline scaffolden. Frameworks kommen automatisch aus Services.",
        "frameworks": "Ziel-Frameworks / Stacks",
        "frameworksPlaceholder": "Ziel-Frameworks wählen",
        "frameworksSynced": "Aus Services synchronisiert",
        "provider": "CI/CD-Anbieter",
        "githubOnly": "Nur GitHub Actions",
        "gitlabOnly": "Nur GitLab CI",
        "securityActive": "SAST & Security-Scans aktiv",
        "verified": "Verifiziert"
      },
      "docker": {
        "label": "Docker",
        "title": "Docker erstellen",
        "blurb": "Multi-Stage-Dockerfiles scaffolden. Stacks kommen automatisch aus Services.",
        "frameworks": "Frameworks / Stacks wählen",
        "frameworksPlaceholder": "FastAPI, Express, React wählen…",
        "frameworksSynced": "Aus Services synchronisiert",
        "hardened": "USER 10001 & Multi-Stage gehärtet",
        "zeroRisks": "0 Risiken"
      },
      "pipelineSecurity": {
        "label": "Pipeline-Security",
        "title": "Lösungen A & B",
        "blurb": "Unabhängig umschalten - nur A, nur B, beide oder keines. Actions sind an Commit-SHAs gepinnt.",
        "solutionA": "Container-Image-Security-Scanning (Lösung A)",
        "solutionABlurb": "Trivy-CVE-Scan nach Build, vor Deploy. Lädt SARIF ins Security-Dashboard.",
        "solutionB": "SAST & Produktionsschutz (Lösung B)",
        "solutionBBlurb": "Statische Analyse vor Build plus Deploy-Health-Check mit Auto-Rollback.",
        "scanner": "Security-Scanner",
        "severity": "Schwellenwert",
        "criticalOnly": "Nur CRITICAL",
        "criticalHigh": "CRITICAL + HIGH (Standard)",
        "onFinding": "Aktion bei Fund",
        "blockDeploy": "Deployment blockieren / Job fehlschlagen",
        "warnUpload": "Warnen & Report hochladen",
        "enableSast": "SAST-Code-Analyse aktivieren (CodeQL / Semgrep)",
        "sastScanner": "SAST-Scanner",
        "codeqlLang": "CodeQL-Sprachpaket",
        "healthRollback": "Automatischer Health-Check & sofortiges Rollback"
      },
      "footerSelection": "Beliebige Kombination aus Provision, Kubernetes und CI/CD wählen. Security-Lösungen A/B gelten nur bei aktiviertem CI/CD und landen in der Pipeline unter ci/.",
      "footerExecution": "Aktionen sind unabhängig. CI/CD neu erzeugen schreibt den Workflow mit aktuellen Security-Toggles (SHA-gepinnte Actions)."
    },
    "frameworkDropdown": {
      "placeholder": "Frameworks / Stacks wählen",
      "searchPlaceholder": "FastAPI, React, Spring suchen…",
      "all": "Alle",
      "noMatch": "Keine passenden Frameworks gefunden.",
      "more": "+{count} weitere",
      "categories": {
        "frontend": "Frontend & Meta-Frameworks",
        "python": "Python-Backends",
        "node": "Node.js-Backends",
        "backend": "JVM, Systems & Generic"
      }
    },
    "containerCard": {
      "title": "Services",
      "blurb": "Frontend- und Backend-Services zum Workspace hinzufügen. Dockerfiles, Compose und CI/CD übernehmen die Stacks automatisch.",
      "servicesLabel": "Services in diesem Workspace",
      "addService": "Service hinzufügen",
      "serviceType": "Service-Typ",
      "frontendUi": "Frontend",
      "backendApi": "Backend",
      "serviceName": "Service-Name",
      "frameworkStack": "Framework-Stack",
      "port": "Port",
      "exposePreview": "Preview-URL",
      "exposePreviewHint": "Aktivierte Services erhalten eine Host-Preview-URL. Deaktivierte Backends bleiben privat im Instanznetzwerk; das Frontend verbindet sich weiterhin über API_URL.",
      "generateDockerfile": "Multi-Stage-Dockerfiles erzeugen (USER 10001)",
      "generateCompose": "docker-compose.yml erzeugen",
      "dockerCompose": "docker-compose.yml",
      "previewToggle": "Generierte Docker-Vorschau",
      "previewHint": "Dockerfile- und Compose-Vorlagen (werden beim Speichern / Provisionieren geschrieben)",
      "syncedToCicd": "Stacks fließen automatisch in Create CI/CD und Create Docker ein."
    },
    "repoSource": {
      "title": "Anwendungs-Repository",
      "blurb": "GitHub- oder GitLab-Repo und Branch für automatische Rebuilds bei Push verknüpfen, oder ein Repo importieren, um einen Workspace aus dem Code zu scaffolden.",
      "modeLink": "Repo verknüpfen",
      "modeImport": "Repo importieren",
      "modeServices": "Services",
      "saveLink": "Verknüpfung speichern",
      "queueLink": "Dieses Repo verwenden",
      "addRepo": "Repository hinzufügen",
      "linkedTitle": "Verknüpfte Repositories",
      "primary": "primär",
      "makePrimary": "Als primär festlegen",
      "primaryHint": "Das primäre Repo wird deployt. Standard ist das Frontend-Repo.",
      "manage": "Verwalten",
      "emptyDetail": "Noch kein Repository verknüpft. Über Aktualisieren verknüpfen oder importieren.",
      "pendingHint": "Repo und Branch jetzt wählen. Launchpad wendet die Verknüpfung an, wenn dieser Workspace erstellt wird.",
      "pendingSaved": "Repo vorgemerkt",
      "pendingSavedBlurb": "Die Verknüpfung wird beim Erstellen oder Speichern des Workspace angewendet.",
      "pendingCleared": "Vorgemerkte Repo-Verknüpfung gelöscht",
      "pendingBadge": "vorgemerkt",
      "loadFailed": "Repository-Verknüpfung konnte nicht geladen werden",
      "saveFailed": "Repository-Verknüpfung konnte nicht gespeichert werden",
      "connectGitlab": "Zuerst GitLab verbinden:",
      "gitlabRepoRequired": "GitLab-Projekt auswählen",
      "gitlabTrackBlurb": "Pushes auf diesen Branch bauen passende Launchpad-Environments neu (GitLab-Webhook). Webhook ggf. unter Integrationen einrichten.",
      "gitlabSaved": "GitLab-Repo wird verfolgt",
      "gitlabSavedBlurb": "Branch-Pushes können Environment-Rebuilds auslösen.",
      "gitlabUnlinked": "GitLab-Tracking entfernt",
      "importBlurb": "Import analysiert ein GitHub- oder GitLab-Repository, erkennt Services und erstellt daraus einen Workspace.",
      "openImport": "Von GitHub / GitLab importieren",
      "saveWorkspaceFirst": "Workspace zuerst speichern oder anlegen, dann ein Repository für branch-basiertes Auto-Deploy verknüpfen."
    },
    "ansible": {
      "label": "Ansible",
      "title": "Host mit Ansible konfigurieren",
      "blurb": "Schreibt Inventory, Playbooks und Rollen nach infra/ansible. Formular, Advanced-YAML oder Cloud-Hinweise nutzen.",
      "enable": "Ansible unter infra/ansible scaffolden",
      "tabForm": "Formular",
      "tabAdvanced": "Advanced",
      "tabCloud": "Cloud",
      "tabAi": "Mit KI aktualisieren",
      "advancedHint": "Generierte Ansible-Dateien bearbeiten. Änderungen fließen in das Scaffold unter infra/ansible beim Speichern ein.",
      "cloudHint": "Inventory auf Cloud-VMs ausrichten. SSH-Benutzer/Schlüssel kommen weiterhin aus dem Formular oder der Running-Instance.",
      "aiBlurb": "Beschreibe, wie der Host laufen soll (PM2, systemd, Docker, nginx/Caddy). Die KI aktualisiert Inventory, Playbook und group_vars unter infra/ansible.",
      "aiContext": "Aktueller Modus: {mode}. Reverse-Proxy: {proxy}.",
      "aiPrompt": "Prompt",
      "aiPromptPlaceholder": "Beispiel: Auf PM2 mit nginx auf Port 80 umstellen, UFW fuer 22/80/443 oeffnen, SSH-Benutzer ubuntu behalten",
      "aiPromptRequired": "Kurzen Prompt eingeben (mindestens 3 Zeichen).",
      "aiUpdate": "Ansible-Dateien aktualisieren",
      "aiUpdating": "Aktualisiere…",
      "aiUpdated": "Ansible-Dateien aktualisiert.",
      "aiFailed": "Ansible-Dateien konnten nicht aktualisiert werden.",
      "cloudProvider": "Ziel-Cloud",
      "cloudProviderHint": "Nutzt den Workspace-Cloud-Provider, wenn gesetzt.",
      "hosts": "Inventory-Hosts",
      "hostsPlaceholder": "127.0.0.1 oder ein Host pro Zeile",
      "group": "Inventory-Gruppe",
      "timezone": "Zeitzone",
      "connection": "SSH-Verbindung",
      "sshUser": "SSH-Benutzer",
      "sshPort": "SSH-Port",
      "sshKey": "Pfad zum privaten SSH-Schlüssel",
      "become": "Become (sudo)",
      "becomeUser": "Become-Benutzer",
      "bootstrap": "Host-Bootstrap",
      "setHostname": "Hostname setzen",
      "hostname": "Hostname",
      "packages": "Basispakete (kommagetrennt)",
      "docker": "Docker",
      "installDocker": "Docker Engine installieren",
      "installCompose": "Compose-Plugin installieren",
      "hardening": "Härtung",
      "ufw": "UFW-Firewall aktivieren",
      "ufwPorts": "UFW erlaubte Ports",
      "fail2ban": "fail2ban aktivieren",
      "unattended": "Unattended Upgrades",
      "app": "Anwendung auf dem Host",
      "appMode": "App-Deploy-Modus",
      "modeDockerRun": "docker run (einzelner Container)",
      "modeCompose": "docker compose",
      "modeSystemd": "systemd-Unit",
      "modePm2": "PM2 (Node)",
      "modeNone": "Nur Bootstrap (keine App-Rolle)",
      "reverseProxy": "Reverse-Proxy",
      "appPort": "App-Listen-Port",
      "appDir": "App-Verzeichnis auf dem Host",
      "syncWorkspace": "Workspace-Quellen ins App-Verzeichnis synchronisieren",
      "deployUser": "Deploy-Benutzer anlegen",
      "fileInventory": "inventory/hosts.yml",
      "filePlaybook": "playbooks/site.yml",
      "fileGroupVars": "group_vars/all.yml",
      "resetAdvanced": "Vom Formular zuruecksetzen",
      "applyCloudDefaults": "Cloud-SSH-Defaults anwenden",
      "syncFromInstance": "Von Running Instance synchronisieren"
    },
    "cost": {
      "label": "Kostenoptimierung",
      "title": "Kostenoptimierung & Right-Sizing",
      "blurb": "Spot-Scheduling, HPA/VPA, Ressourcen-Presets und Idle-Shutdown in infra/-Manifeste injizieren.",
      "spot": "Spot- / Preemptible-VM-Scheduling aktivieren",
      "spotBlurb": "Spot-Kapazität mit Node-Affinity und Tolerations bevorzugen.",
      "spotPlacement": "Ziel-Workload-Platzierung",
      "spotAllocation": "Spot-Allokation - {percent} %",
      "spotProvisioner": "Node-Scaler / Provisioner",
      "hpa": "Horizontal Pod Autoscaling (HPA) aktivieren",
      "hpaBlurb": "Schreibt hpa.yaml mit CPU-Zielauslastung.",
      "vpa": "Vertical Pod Autoscaler (Empfehlungsmodus) aktivieren",
      "vpaBlurb": "VPA updateMode: Off - Nutzung beobachten ohne Restarts.",
      "resources": "Ressourcen-Limits & Quotas",
      "idleShutdown": "Geplantes Workspace-Sleep / Auto-Teardown aktivieren",
      "idleBlurb": "Deployment außerhalb der Arbeitszeiten auf 0 skalieren (Mo-Fr 19-7 Uhr + Wochenenden).",
      "schedule": "Zeitplan",
      "presets": {
        "developer": "Developer / Lean",
        "balanced": "Balanced",
        "performance": "Performance",
        "custom": "Custom"
      }
    },
    "dependencies": {
      "title": "Workload-Abhängigkeiten",
      "blurb": "Datenbanken und Caches als Companion-Services im Cluster hinzufügen oder Managed-Cloud-Ressourcen verbinden.",
      "inCluster": "Im Cluster",
      "managedCloud": "Managed Cloud",
      "enableManagedFirst": "Zuerst {service} in Cloud-Ressourcen aktivieren.",
      "managedHint": "Connection-Strings nutzen Terraform-Outputs - siehe infra/MANAGED_DATASTORES.md nach Apply.",
      "brokersTitle": "Message-Broker",
      "brokersBlurb": "Broker im Cluster für deine Microservices betreiben oder eigenen Broker angeben. Services erhalten KAFKA_BROKERS / RABBITMQ_URL automatisch.",
      "bringYourOwn": "Extern (eigene URL)",
      "brokerUrl": "Broker-URL",
      "connectionUrl": "Verbindungs-URL",
      "inClusterAutoInjected": "Launchpad betreibt dies im Cluster und injiziert die Verbindungs-URL automatisch als Umgebungsvariable (DATABASE_URL / REDIS_URL). Keine Einrichtung nötig."
    },
    "detected": {
      "architecture": "Erkannte Architektur",
      "previewTarget": "Preview-Ziel",
      "include": "Einbeziehen",
      "port": "Port",
      "primaryPreview": "Primäre Preview",
      "setPreviewTarget": "Als Preview-Ziel setzen",
      "noDockerfile": "Kein Dockerfile - Launchpad scaffoldet beim Speichern eines",
      "dockerfilePresent": "Dockerfile: {path}"
    },
    "setup": {
      "label": "Service-Setup",
      "step": "SCHRITT {current}/{total}",
      "step1": "Infrastruktur-Outputs wählen",
      "step2": "Service-Ressourcen bearbeiten",
      "step3": "Workspace-Setup prüfen und speichern",
      "loading": "Setup wird geladen…",
      "detectedTitle": "Aus Workspace erkannt",
      "detectedBlurb": "Schalter unten wurden aus vorhandenen Dateien aktiviert.",
      "credentialsStored": "Kontoschlüssel aus den Einstellungen (oder diesem Workspace) wurden erkannt. Felder leer lassen, um sie zu nutzen, oder neue Werte einfügen zum Überschreiben.",
      "clusterName": "Cluster-Name",
      "kubectlContext": "kubectl-Kontext",
      "servicesInIac": "In IaC einzubeziehende Services",
      "selectServicesFirst": "Als Nächstes Cloud-Services auswählen, dann Infrastruktur-Generierung aktivieren (Kubernetes wird freigeschaltet, wenn GKE, EKS oder AKS ausgewählt ist).",
      "reviewWorkspace": "Workspace",
      "reviewProvider": "Anbieter",
      "reviewProvision": "Provision",
      "reviewKubernetes": "Kubernetes",
      "reviewCicd": "CI/CD",
      "reviewRunInit": "Init ausführen",
      "reviewCloudServices": "Cloud-Services in IaC",
      "localK8s": "lokales k8s",
      "noneSelected": "Keine ausgewählt",
      "skipped": "übersprungen",
      "saveSetup": "Workspace-Setup speichern",
      "updated": "Workspace-Setup aktualisiert."
    }
  },
  "analyzer": {
    "preview": {
      "eyebrow": "Launch-Preview-Analyzer",
      "title": "Diagnosebericht",
      "closeLabel": "Analyzer schließen",
      "loading": "CI/CD, Kubernetes, Trivy und CodeQL-Telemetrie werden analysiert…",
      "summary": "Zusammenfassung",
      "securityDetails": "Security-Details",
      "cveRule": "CVE / Regel",
      "affected": "Betroffen",
      "recommendedFix": "Empfohlener Fix",
      "rootCause": "Ursache",
      "actionableSteps": "Umsetzbare Schritte",
      "proposedFix": "Vorgeschlagener Fix",
      "copyDiff": "Diff kopieren",
      "applyPrompt": "Fix im verknüpften Workspace anwenden oder ablehnen?",
      "noWorkspace": "Kein verknüpfter Workspace - workspace-basierte Preview öffnen oder Diff manuell kopieren.",
      "applyFix": "Fix anwenden",
      "reject": "Ablehnen",
      "fixApplied": "Fix angewendet.",
      "fixRejected": "Fix abgelehnt.",
      "openWorkspace": "Workspace öffnen",
      "telemetry": "Telemetrie erfasst",
      "empty": "Analyze failure ausführen, um eine strukturierte Diagnose aus Preview-Logs und Security-Scans zu erzeugen.",
      "categories": {
        "containerVuln": "Container-Schwachstelle",
        "sast": "SAST / Code-Security",
        "runtimeCrash": "Runtime-Crash",
        "configError": "Konfigurationsfehler"
      }
    },
    "workspace": {
      "eyebrow": "KI-Analyse",
      "workspaceFile": "Workspace-Datei",
      "filesCount": "{count} Dateien",
      "scanning": "Scannen…",
      "issueCountOne": "{count} Problem",
      "issueCountMany": "{count} Probleme",
      "fixable": " · {count} behebbar",
      "sandboxContext": "Sandbox-Fehlerkontext wird für den Fix genutzt.",
      "autoDetect": "Domäne auto-erkennen",
      "cicd": "CI/CD",
      "docker": "Docker",
      "iac": "IaC",
      "kubernetes": "Kubernetes",
      "rerun": "Erneut ausführen",
      "analyzing": "Analysieren…",
      "reviewingOne": "{count} Datei wird geprüft…",
      "reviewingMany": "{count} Dateien werden geprüft…",
      "issues": "Probleme",
      "suggestions": "Vorschläge",
      "suggestedRewrite": "Vorgeschlagene Überarbeitung",
      "applyFixWorkspace": "Fix im Workspace anwenden",
      "applyFixFile": "Fix in Datei anwenden",
      "targetFile": "Zieldatei:",
      "targetBlurb": "Zieldatei auf Fehler analysiert. Unten klicken, um Fixes direkt anzuwenden.",
      "fixableFooterOne": "{count} Datei mit vorgeschlagenen Fixes",
      "fixableFooterMany": "{count} Dateien mit vorgeschlagenen Fixes",
      "applyAllFixes": "Alle Fixes anwenden",
      "fix": "Fix"
    }
  },
  "jobLog": {
    "title": "Provisionierungs-Logs",
    "analyzeFailure": "Fehler analysieren",
    "analyzing": "Analysieren…",
    "streaming": "streamt",
    "complete": "abgeschlossen",
    "idle": "inaktiv",
    "waiting": "Warte auf Worker-Ausgabe…"
  },
  "dockerfiles": {
    "title": "Repository-Scaffold",
    "blurb": "Importierten Workspace scannen und Dockerfiles / CI-Scaffolds erzeugen.",
    "scan": "Repository scannen",
    "scaffoldSelected": "Auswahl scaffolden",
    "aiReview": "KI-Sicherheitsprüfung",
    "applyImproved": "Verbesserte Version anwenden",
    "failed": "Scaffold fehlgeschlagen",
    "pageTitle": "Repo-Scaffolding",
    "repository": "Repository",
    "githubInstallation": "GitHub-Installation",
    "selectInstallation": "Installation auswählen",
    "selectRepository": "Repository auswählen",
    "branchRef": "Branch / Ref",
    "detectedStack": "Erkannter / Scaffold-Stack",
    "auto": "Auto",
    "appName": "App-Name",
    "scanning": "Scannen…",
    "scaffolding": "Scaffolding…",
    "dockerfileOnly": "Nur Dockerfile",
    "scaffoldArtifacts": "Scaffold-Artefakte",
    "scaffoldArtifactsBlurb": "Wähle, was aus dem gescannten Repo-Stack erzeugt wird",
    "scaffoldFiles": "Scaffold-Dateien",
    "scanOrScaffold": "Repo scannen oder ausgewählte Artefakte scaffolden.",
    "editor": "Editor",
    "download": "Herunterladen",
    "reviewing": "Prüfen…",
    "loadingEditor": "Editor wird geladen…",
    "securityReview": "Sicherheitsprüfung",
    "changesImproved": "Änderungen im verbesserten Dockerfile",
    "pushToGithub": "Nach GitHub pushen",
    "pushBlurb": "Committed Scaffold-Dateien unter dockers/, ci/ und infra/ über die GitHub App.",
    "activeFilePath": "Aktiver Dateipfad",
    "commitMessage": "Commit-Nachricht",
    "pushing": "Pushen…",
    "pushAll": "Alles pushen ({count})",
    "pushActiveFile": "Aktive Datei pushen",
    "buildPushImage": "Image bauen & pushen",
    "buildBlurb": "Klont das Repo, baut mit Docker und pusht in die gewählte Registry. Erfordert laufenden Worker (make worker) und Docker-Daemon.",
    "dockerfilePath": "Dockerfile-Pfad",
    "buildContext": "Build-Kontext",
    "tagsLabel": "Tags (Komma oder Leerzeichen getrennt)",
    "username": "Benutzername",
    "passwordToken": "Passwort / Access Token",
    "repoUserName": "Repository (user/name)",
    "accessKeyId": "Access Key ID",
    "secretAccessKey": "Secret Access Key",
    "sessionTokenOptional": "Session Token (optional)",
    "accountId": "Account ID",
    "ecrRepository": "ECR-Repository",
    "serviceAccountJson": "Service-Account-JSON",
    "location": "Standort",
    "arRepository": "AR-Repository",
    "imageName": "Image-Name",
    "building": "Bauen…",
    "buildPush": "Bauen & pushen",
    "removeFileTitle": "Datei entfernen?",
    "removeFileConfirm": "\"{path}\" aus dieser Scaffold-Sitzung entfernen?",
    "removeFromSession": "Aus Sitzung entfernen",
    "stackLabel": "Stack",
    "refLabel": "Ref",
    "markersLabel": "Marker",
    "none": "keine",
    "line": "Zeile"
  },
  "workspaceIde": {
    "title": "Workspace-IDE",
    "publish": "Veröffentlichen",
    "explorer": "Explorer",
    "addYaml": "Kubernetes-YAML hinzufügen",
    "addTerraform": "Terraform-Datei hinzufügen",
    "download": "Herunterladen",
    "format": "Formatieren",
    "saved": "Gespeichert",
    "save": "Speichern",
    "saving": "Speichern…",
    "aiAnalyze": "KI verbessern",
    "aiAnalyzeFolder": "Ordner mit KI verbessern",
    "aiAnalyzeFolderTitle": "Alle analysierbaren Dateien im Ordner verbessern (CI, Docker, Terraform)",
    "aiAnalyzeFileTitle": "Ausgewählte CI-, Docker- oder Terraform-Datei mit KI verbessern",
    "kubernetes": "Kubernetes",
    "engineLabel": "Engine:",
    "restoreFiles": "Dateien wiederherstellen",
    "restoring": "Wiederherstellen…",
    "noFileOpen": "Keine Datei geöffnet",
    "folderLabel": "Ordner: {path}",
    "selectFileHint": "Datei im Explorer wählen zum Bearbeiten",
    "untitled": "Unbenannt",
    "newFileTitle": "Neue Datei",
    "newFolderTitle": "Neuer Ordner",
    "refreshTitle": "Aktualisieren",
    "dialogs": {
      "newFile": "Neue Datei",
      "newFolder": "Neuer Ordner",
      "rename": "Umbenennen",
      "deletePath": "Pfad löschen?",
      "deletePathMessage": "\"{path}\" aus diesem Workspace löschen? Dies kann nicht rückgängig gemacht werden.",
      "discardUnsaved": "Ungespeicherte Änderungen verwerfen?",
      "discardMessage": "Ungespeicherte Änderungen in der aktuellen Datei. Beim Öffnen einer anderen Datei gehen sie verloren.",
      "discardConfirm": "Verwerfen und öffnen",
      "keepEditing": "Weiter bearbeiten",
      "destroyResources": "Ressourcen zerstören?"
    },
    "push": {
      "title": "Workspace veröffentlichen",
      "blurb": "Neues Repository erstellen oder in ein bestehendes pushen. Bei CI-Mismatch wird zuerst konvertiert (GitHub ↔ GitLab).",
      "connectGithub": "Zuerst GitHub verbinden -",
      "connectGitlab": "Zuerst GitLab verbinden -",
      "integrations": "Integrationen",
      "createRepo": "Neues Repository erstellen",
      "existingRepo": "Bestehendes Repository",
      "createProject": "Neues Projekt erstellen",
      "existingProject": "Bestehendes Projekt",
      "personalGithubWarning": "Persönliche GitHub-Konten können über App-Installationstokens keine neuen Repos erstellen. Bevorzuge Bestehendes Repository oder lege zuerst ein leeres Repo auf GitHub an. Org-Installationen können Repos direkt erstellen.",
      "newRepoName": "Neuer Repository-Name",
      "useWorkspaceName": "Workspace-Namen nutzen",
      "createsUnder": "Erstellt unter",
      "privateRepo": "Privates Repository",
      "repository": "Repository",
      "newProjectName": "Neuer Projektname",
      "privateProject": "Privates Projekt",
      "project": "Projekt",
      "selectProject": "Projekt auswählen",
      "orPath": "Oder Pfad mit Namespace",
      "commitMessage": "Commit-Nachricht",
      "publishing": "Veröffentlichen…",
      "createPushGithub": "Erstellen & nach GitHub pushen",
      "pushGithub": "Nach GitHub pushen",
      "createPushGitlab": "Erstellen & nach GitLab pushen",
      "pushGitlab": "Nach GitLab pushen",
      "installation": "Installation",
      "toastSuccess": "Erfolgreich veröffentlicht",
      "toastError": "Veröffentlichung fehlgeschlagen",
      "toastFailed": "Push fehlgeschlagen",
      "toastStatusFailed": "GitHub-Status konnte nicht geladen werden",
      "convertedToGithub": "GitLab CI vor dem Veröffentlichen in GitHub Actions konvertiert",
      "convertedToGitlab": "GitHub Actions vor dem Veröffentlichen in GitLab CI konvertiert"
    },
    "toolbar": {
      "iacLabel": "IaC",
      "blurb": "Provision führt init → validate → plan → apply geführt aus. Destroy fragt zuerst nach Bestätigung.",
      "blurbWithAnsible": "Cloud-Stack mit Terraform/Pulumi provisionieren, danach Ansible auf dem gewählten Cloud-Host ausführen.",
      "ansibleBlurb": "Ansible installiert Collections, macht einen Dry-Run und wendet playbooks/site.yml für die gewählte Cloud an.",
      "opensSandbox": "(öffnet Sandbox bei Bedarf)",
      "provisionStack": "Stack provisionieren",
      "runAnsible": "Ansible ausführen",
      "runAnsibleForCloud": "Ansible · {cloud}",
      "runAnsibleDescription": "Hosts auf {cloud} mit infra/ansible konfigurieren (galaxy → check → playbook)",
      "ansibleDestroyHint": "Ansible baut keine Cloud-Infra ab; dafür Terraform oder Pulumi Destroy nutzen",
      "provisionDescription": "{engine} init → validate → plan → apply in der Sandbox ausführen",
      "destroyDescription": "{engine}-verwaltete Cloud-Ressourcen abbauen"
    },
    "engines": {
      "terraform": "Terraform",
      "opentofu": "OpenTofu",
      "pulumi": "Pulumi",
      "ansible": "Ansible"
    },
    "initModal": {
      "provisionTitle": "Provision mit {engine}",
      "destroyTitle": "Destroy mit {engine}",
      "ansibleTitle": "Ansible auf {cloud} ausführen",
      "provisionBlurb": "Services prüfen, Cloud-Key wählen, dann init → validate → plan → apply ausführen (auto-genehmigt).",
      "destroyBlurb": "Cloud-Key bestätigen, dann verwaltete Ressourcen zerstören (auto-genehmigt nach Bestätigung).",
      "ansibleBlurb": "Hosts für {cloud} prüfen, SSH/Credentials bestätigen, dann galaxy → check → ansible-playbook playbooks/site.yml.",
      "loadingConfig": "Workspace-Konfiguration wird geladen…",
      "continueToDestroy": "Weiter zu Destroy",
      "continueToProvision": "Weiter zu Provision",
      "continueToAnsible": "Weiter zu Ansible",
      "done": "Fertig"
    }
  },
  "hybrid": {
    "eyebrow": "Fleet",
    "title": "Fleet",
    "subtitle": "Selbst gehostete Hosts einschreiben und per KI auf einen Knoten oder in die Cloud deployen.",
    "nodes": {
      "title": "Deployment-Nodes",
      "subtitle": "Selbst gehostete Agenten über einen sicheren Reverse-Tunnel verbunden.",
      "empty": "Noch keine Nodes. Registriere einen selbst gehosteten Host.",
      "enroll": "Node registrieren",
      "enrollTitle": "Neuen Node registrieren",
      "nameLabel": "Node-Name",
      "namePlaceholder": "z. B. homelab-nas",
      "create": "Installations-Token erstellen",
      "installTitle": "Agent installieren",
      "installHint": "Auf dem Host ausführen. Docker erforderlich. Der Token wird nur einmal angezeigt und läuft bald ab.",
      "tokenOnce": "Token jetzt speichern, er wird nicht erneut angezeigt.",
      "copy": "Kopieren",
      "copied": "Kopiert",
      "done": "Fertig",
      "revoke": "Widerrufen",
      "revokeConfirm": "Diesen Node aus Launchpad entfernen? Der Agent wird getrennt und der Eintrag gelöscht.",
      "online": "Online",
      "offline": "Offline",
      "pending": "Ausstehend",
      "revoked": "Widerrufen",
      "cpu": "CPU",
      "mem": "Speicher",
      "disk": "Festplatte",
      "docker": "Docker",
      "containers": "Container",
      "lastSeen": "Zuletzt gesehen",
      "never": "nie",
      "platform": "Plattform",
      "version": "Agent"
    },
    "ai": {
      "title": "KI-Infrastruktur-Provisionierung",
      "subtitle": "Beschreibe den gewünschten Stack. Wir erzeugen ein abgesichertes Blueprint und eine Kostenschätzung.",
      "promptLabel": "Beschreibe deine Infrastruktur",
      "promptPlaceholder": "Redis-Cache mit 1 GB RAM und einen FastAPI-Worker mit persistentem Speicher bereitstellen",
      "target": "Ziel",
      "targetNode": "Selbst gehosteter Node",
      "targetGcp": "Google Cloud",
      "targetAws": "AWS",
      "targetAzure": "Azure",
      "nodeLabel": "Node",
      "selectNode": "Node auswählen",
      "region": "Region (optional)",
      "generate": "Blueprint erzeugen",
      "generating": "Wird erzeugt…",
      "deploy": "Bereitstellen",
      "deploying": "Wird bereitgestellt…",
      "geminiOn": "Gemini verbunden",
      "geminiOff": "Gemini nicht konfiguriert (Heuristik-Modus)",
      "sourceGemini": "Von Gemini erzeugt",
      "sourceHeuristic": "Per Heuristik erzeugt",
      "blueprint": "Blueprint",
      "services": "Dienste",
      "guardrails": "Schutzregeln",
      "valid": "Gültig",
      "invalid": "Blockiert",
      "adjusted": "Automatisch an Limits angepasst",
      "cost": "Geschätzte Kosten",
      "hourly": "Stündlich",
      "monthly": "Monatlich",
      "selfHosted": "Selbstgehostet (Rechen-Äquivalent-Schätzung)",
      "deployLog": "Deployment-Protokoll",
      "noBlueprint": "Erzeuge ein Blueprint, um Dienste, Schutzregeln und Kosten zu sehen.",
      "deploySuccess": "Deployment abgeschlossen.",
      "deployFailed": "Deployment mit Fehlern beendet.",
      "needNode": "Wähle einen Online-Node zum Bereitstellen.",
      "fixWithAi": "Mit KI beheben",
      "fixing": "Wird behoben…",
      "fixApplied": "Blueprint repariert. Dienste prüfen und erneut bereitstellen.",
      "fixFailed": "Blueprint konnte nicht repariert werden.",
      "fixHint": "Das Deployment ist fehlgeschlagen. Mit KI beheben schreibt kaputte Image-Tags und andere Blueprint-Probleme um; danach erneut bereitstellen.",
      "nextSteps": "Weiter",
      "viewDeployment": "Deployment ansehen",
      "viewWorkspace": "Workspace öffnen",
      "viewNode": "Node ansehen",
      "openService": "{name} öffnen",
      "deployedOnNode": "Deployed auf Node {name}",
      "deployedServices": "Laufende Dienste",
      "noServiceUrl": "Keine veröffentlichte URL (Containerliste am Node prüfen)",
      "dismissResult": "Schließen"
    }
  },
  "serviceGraph": {
    "title": "Dienstverbindungen",
    "subtitle": "Wie die Dienste in diesem Workspace kommunizieren. Erkannte Verbindungen werden automatisch angezeigt; eigene hinzufügen, um sie zu überschreiben oder zu ergänzen.",
    "legend": "Durchgezogene Verbindungen sind von dir konfiguriert. Gepunktete Verbindungen wurden aus dem Code erkannt.",
    "connections": "Verbindungen",
    "none": "Noch keine Verbindungen erkannt.",
    "configured": "konfiguriert",
    "auto": "erkannt",
    "addConnection": "Verbindung hinzufügen",
    "source": "Von",
    "target": "Nach",
    "protocol": "Protokoll",
    "protocolAuto": "Automatisch aus Datenbank/Broker erkannt",
    "add": "Verbindung hinzufügen",
    "saving": "Speichern…",
    "remove": "Entfernen"
  }
} as const
