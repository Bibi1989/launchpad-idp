...
class WorkspaceGenerator:
    ...
    def generate(
        ...
        *,
        ...
        if mode == "docker_compose":
            ...
            # Dockerfile(s) only; deploy path is attach/local Docker by default.
            # Process strategy scaffolds (pm2/systemd/nginx) are written by the
            # import/provision callers via write_instance_process_scaffold.
            pass
        else:
        ...
        env_written = self._write_workspace_env(workspace_dir, env_vars or {})
        if env_written:
            result.files.extend(env_written)
    ...
    def _write_workspace_env(
        ...
        """Write root ``.env`` from import-time configuration (gitignored)."""
        if not env_vars:
            return []
        lines = [
            "# Generated by Launchpad import. Do not commit secrets.",
            "# Sourced from .env.example defaults + your overrides.",
        ]
        for key in sorted(env_vars.keys()):
            value = env_vars[key]
            # Quote when needed for dotenv safety
        ...
        ignore_blob = gitignore.read_text(encoding="utf-8") if gitignore.is_file() else ""
        if ".env" not in ignore_blob.splitlines() and not re.search(
            r"(?m)^\s*\.env\s*$", ignore_blob
        ):
            suffix = "" if ignore_blob.endswith("\n") or not ignore_blob else "\n"
            gitignore.write_text(
                ignore_blob + suffix + "\n# Launchpad import secrets\n.env\n.env.local\n",
                encoding="utf-8",
            )
    ...
    def _write_k8s_manifests(
        ...
        *,
        ...
        if isinstance(dependencies, WorkloadDependenciesConfig):
            deps = dependencies
        else:
