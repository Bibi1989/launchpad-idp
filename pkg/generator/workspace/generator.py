...
class WorkspaceGenerator:
    ...
    def generate(
        ...
        *,
        ...
        mode = (runtime_mode or "kubernetes").strip().lower()
        ...
        if mode == "docker_compose":
            compose = self._write_compose_file(
                workspace_dir,
        ...
        elif mode == "running_instance":
            # Dockerfile(s) only; deploy path is attach/local Docker.
            pass
        else:
        ...
        readme.write_text(
            self._readme(workspace_name, active, detection, runtime_mode=mode),
            encoding="utf-8",
        )
    ...
    def _write_compose_file(
        ...
        *,
        ...
        if existing.is_file() or (workspace_dir / "compose.yaml").is_file():
            return None
        lines = [
            f"# Launchpad import compose for {workspace_name}",
            "services:",
        ]
        for svc in services:
            svc_key = re.sub(r"[^a-z0-9]+", "-", svc.name.lower()).strip("-") or "app"
            port = svc.port or 8080
            build_ctx = "." if svc.path in {".", ""} else svc.path
            lines.extend(
                [
                    f"  {svc_key}:",
                    f"    build: {build_ctx}",
...
"""
    ...
    def _readme(
        ...
        *,
        ...
        if runtime_mode == "docker_compose":
            next_steps = [
                "",
                "## Next steps",
                "",
                "1. Review `docker-compose.yml` and Dockerfiles.",
                "2. Open Launchpad → Launch and select this workspace (Compose deploy).",
                "3. Or push to GitHub/GitLab from the workspace page.",
                "",
            ]
        elif runtime_mode == "running_instance":
            next_steps = [
                "",
                "## Next steps",
                "",
                "1. Review Dockerfiles (and optional IaC under `infra/`).",
                "2. Open Launchpad → Launch and select this workspace (running instance).",
                "3. Or push to GitHub/GitLab from the workspace page.",
                "",
            ]
        else:
